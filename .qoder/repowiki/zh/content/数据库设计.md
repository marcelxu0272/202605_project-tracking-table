...
- **新增**：报告行管理系统数据库架构，包括 report_lines、report_line_pm_status、report_line_approvals、report_line_data 四个新表，支持多级审批工作流和项目跟踪
...
- **新增**：server/report-line-service.js：报告行管理服务，处理多级审批流程与数据同步
- **新增**：server/report-line-seed.js：报告行演示数据种子，提供多级审批流程的模拟数据
...
- **新增**：js/views/ReportLineDetail.js：报告行详情视图，支持 Luckysheet 编辑器集成
- **新增**：js/views/ReportLineList.js：报告行列表视图，支持筛选、分页与操作按钮
...
RLSvc["报告行服务<br/>server/report-line-service.js"]
AlertSvc["预警服务<br/>server/alert-service.js"]
AlertSeed["预警种子<br/>server/alert-demo-seed.js"]
RLDetail["报告行详情<br/>js/views/ReportLineDetail.js"]
RLList["报告行列表<br/>js/views/ReportLineList.js"]
end
subgraph "数据层"
...
RLines["表: report_lines<br/>报告行主表"]
RLPm["表: report_line_pm_status<br/>PM提交状态"]
RLApp["表: report_line_approvals<br/>审批记录"]
RLData["表: report_line_data<br/>报告行项目数据"]
...
- [server/report-line-service.js:1-336](file://server/report-line-service.js#L1-L336)
...
- [js/views/ReportLineDetail.js:1-372](file://js/views/ReportLineDetail.js#L1-L372)
- [js/views/ReportLineList.js:1-493](file://js/views/ReportLineList.js#L1-L493)

**章节来源**
- [database-schema.sql:1-413](file://database-schema.sql#L1-L413)
- [server/db.js:1-667](file://server/db.js#L1-L667)
- [server/index.js:1-775](file://server/index.js#L1-L775)
...
- **新增**：报告行主表 report_lines：按板块×周期创建的报告行，支持多级审批流程
- **新增**：PM提交状态表 report_line_pm_status：记录报告行内各PM的提交状态
- **新增**：审批记录表 report_line_approvals：记录报告行审批流程的每一步操作
- **新增**：报告行项目数据表 report_line_data：存储报告行内各项目的字段数据与相对baseline的变更差异
...
- [database-schema.sql:337-348](file://database-schema.sql#L337-L348)
- [database-schema.sql:358-367](file://database-schema.sql#L358-L367)
- [database-schema.sql:376-388](file://database-schema.sql#L376-L388)
- [database-schema.sql:397-408](file://database-schema.sql#L397-L408)
...
participant RLSvc as "报告行服务<br/>server/report-line-service.js"
...
- [server/report-line-service.js:83-299](file://server/report-line-service.js#L83-L299)
...
- 字段定义与约束：
  - 主键与唯一：project_no 唯一
  - 系统同步字段：sector_code、pm_name、project_name、client_name、enterprise_type、industry、business_type、start_date、end_date
...
### **新增** 报告行主表 report_lines
- 设计目的：按板块×周期创建的报告行，支持多级审批流程
- 字段定义与约束：
  - 主键：id
  - 字段：sector_code、period、status、approval_node、baseline_version、distributed_columns、created_at、updated_at
  - 唯一约束：(sector_code, period)
  - 状态枚举：open、reviewing_director、reviewing_leader、completed、rejected、closed
  - 分发列配置：distributed_columns（JSON，列字母数组，null表示显示全部列）
- 索引：sector_code、period、status
- 数据完整性：唯一约束确保每板块每周期只能有一个报告行
...
**章节来源**
- [database-schema.sql:337-348](file://database-schema.sql#L337-L348)
- [server/db.js:83-96](file://server/db.js#L83-L96)

### **新增** PM提交状态表 report_line_pm_status
- 设计目的：记录报告行内各PM的提交状态
- 字段定义与约束：
  - 主键：id
  - 外键：report_line_id → report_lines.id
  - 字段：report_line_id、pm_name、status、submitted_at
  - 唯一约束：(report_line_id, pm_name)
  - 状态枚举：open、submitted、closed
- 索引：report_line_id、status
- 数据完整性：外键约束，唯一约束确保每个PM在报告行中只能有一条状态记录
...
REPORT_LINES ||--o{ REPORT_LINE_PM_STATUS : "外键: report_line_id"
```
...
**章节来源**
- [database-schema.sql:358-367](file://database-schema.sql#L358-L367)
- [server/db.js:97-107](file://server/db.js#L97-L107)

### **新增** 审批记录表 report_line_approvals
- 设计目的：记录报告行审批流程的每一步操作
- 字段定义与约束：
  - 主键：id
  - 外键：report_line_id → report_lines.id
  - 字段：report_line_id、action、actor_role、actor_name、comment、from_status、to_status、snapshot_version、created_at
  - 动作枚举：submit、approve、reject、auto_skip、close_pm
  - 快照版本：snapshot_version（板块管理员提交时写入 D 版快照键）
- 索引：report_line_id、action
- 数据完整性：外键约束
...
REPORT_LINES ||--o{ REPORT_LINE_APPROVALS : "外键: report_line_id"
```
...
**章节来源**
- [database-schema.sql:376-388](file://database-schema.sql#L376-L388)
- [server/db.js:108-121](file://server/db.js#L108-L121)

### **新增** 报告行项目数据表 report_line_data
- 设计目的：存储报告行内各项目的字段数据与相对baseline的变更差异
- 字段定义与约束：
  - 主键：id
  - 外键：report_line_id → report_lines.id
  - 字段：report_line_id、project_no、field_data（JSON）、change_diff（JSON）、updated_by、updated_at
  - 唯一约束：(report_line_id, project_no)
  - 字段数据：field_data（JSON）存储项目字段数据
  - 变更差异：change_diff（JSON）存储相对baseline的变更
- 索引：report_line_id、project_no
- 数据完整性：外键约束，唯一约束确保每个项目在报告行中只能有一条数据记录
...
REPORT_LINES ||--o{ REPORT_LINE_DATA : "外键: report_line_id"
PROJECTS ||--o{ REPORT_LINE_DATA : "外键: project_no"
```
...
**章节来源**
- [database-schema.sql:397-408](file://database-schema.sql#L397-L408)
- [server/db.js:122-135](file://server/db.js#L122-L135)
...
- **新增**：报告行主表 report_lines 与项目主表无直接外键，但通过 report_line_data 关联
- **新增**：PM提交状态表 report_line_pm_status 与报告行主表一对多
- **新增**：审批记录表 report_line_approvals 与报告行主表一对多
- **新增**：报告行项目数据表 report_line_data 与报告行主表一对多，与项目主表一对多
...
RLines["report_lines"] --> RLPm["report_line_pm_status"]
RLines --> RLApp["report_line_approvals"]
RLines --> RLData["report_line_data"]
RLData --> Projects
```
...
- [database-schema.sql:254-268](file://database-schema.sql#L254-L268)
- [database-schema.sql:279-286](file://database-schema.sql#L279-L286)
- [database-schema.sql:337-408](file://database-schema.sql#L337-L408)
- [database-schema.sql:295-331](file://database-schema.sql#L295-L331)
- [server/db.js:29-32](file://server/db.js#L29-L32)
...
- 索引策略
  ...
  - **新增**：report_lines：按板块、周期、状态建立索引，支持报告行快速查询
  - **新增**：report_line_pm_status：按报告行、状态建立索引，支持PM状态查询
  - **新增**：report_line_approvals：按报告行、动作建立索引，支持审批历史查询
  - **新增**：report_line_data：按报告行、项目号建立索引，支持项目数据快速访问
...
- JSON 存储
  ...
  - **新增**：report_line_data.field_data 与 report_line_data.change_diff 使用 JSON 存储，支持灵活的数据结构
- WAL 模式
  - 启用 WAL 模式提升并发读写性能
- 查询优化
  - 月度数据初始化与跨年翻转采用批量 INSERT/UPDATE，减少事务次数
  - 审计日志按月度与操作人索引，避免全表扫描
  - **新增**：报告行查询通过多级索引优化，支持按板块、周期、状态的快速筛选
...
- [database-schema.sql:350-353](file://database-schema.sql#L350-L353)
- [database-schema.sql:369-371](file://database-schema.sql#L369-L371)
- [database-schema.sql:390-392](file://database-schema.sql#L390-L392)
- [database-schema.sql:410-411](file://database-schema.sql#L410-L411)
...
- **新增**：报告行系统故障排查
  - 报告行状态异常：检查 report_lines 表的状态字段与 report_line_approvals 的审批记录
  - PM提交状态不正确：检查 report_line_pm_status 表中各PM的状态是否与实际相符
  - 审批流程中断：检查 report_line_approvals 表中最近的审批记录，确认审批权限与状态转换
  - 项目数据缺失：检查 report_line_data 表中项目数据是否正确同步
  - 报告行重复创建：检查 report_lines 表中是否有多条相同 (sector_code, period) 的记录
  - 分发列配置问题：检查 report_lines.distributed_columns 是否正确解析为列集合
- **新增**：预警系统故障排查
...
- 审计双层设计与快照不可变性
- 板块工作流与版本控制
- **新增**：完善的报告行管理系统，支持多级审批流程、PM提交状态跟踪、审批记录审计与项目数据管理
- **新增**：完善的项目预警系统，支持多种预警类型的检测、状态管理和永久忽略机制

在性能方面，通过索引与 WAL 模式提升查询与并发能力；在可维护性方面，提供快照维护与元数据管理工具链。**新增**的报告行管理系统通过专门的索引优化了多级审批流程的查询性能，并提供了灵活的状态管理机制。**新增**的预警系统通过专门的索引优化了查询性能，并提供了灵活的状态管理机制。未来可进一步引入软删除、乐观锁与分区策略以增强扩展性与稳定性。
...
- 备份与恢复
  ...
  - **新增**：报告行数据备份：report_lines、report_line_pm_status、report_line_approvals、report_line_data 表同样重要，需要纳入常规备份策略
  - **新增**：预警数据备份：project_alerts 和 project_alert_dismissals 表同样重要，需要纳入常规备份策略
- 维护建议
  - 定期清理遗留快照键
  - 监控 WAL 文件大小与磁盘空间
  - 通过 API 触发平台同步与快照生成，确保数据时效性
  - **新增**：监控报告行服务运行状态，定期检查报告行数据的完整性和准确性
  - **新增**：监控预警服务运行状态，定期检查预警数据的完整性和准确性
  - **新增**：定期清理过期的报告行记录和预警记录，保持数据库性能
