...
## 更新摘要
**已进行的更改**
- 新增项目数据范围判定规则：明确按板块/项目经理查看的数据范围判定
- 更新 J 版归档程序：仅在锁定期可用，版本切换下拉框仅系统管理员可见
- 新增库存验证流程：手工完成额实时拦截 S<0，系统同步自动对冲
- 新增手动永久忽略预警功能：系统管理员可永久忽略确认无需处理的预警
- 更新库存验证规则：调整存量合同额校验逻辑，明确系统管理员可编辑列范围
- 新增初始化导入与平台数据合并规则
...
**更新** 新增业务规则澄清：明确项目数据范围判定规则、J版本归档程序、库存验证流程、手动永久忽略预警等功能。
...
- 风险评估：平台同步失败、导入合并冲突、审批流程阻塞、数据一致性风险、邮件提醒服务异常、J版归档失败、库存验证错误、预警忽略失效
- 变更控制策略：变更写入审计日志、字段字典版本锁定、平台同步与导入合并规则、J 版归档后重置流程态、动态时间窗配置、手动永久忽略预警

**更新** 新增了 J 版归档失败、库存验证错误、预警忽略失效等风险评估。
