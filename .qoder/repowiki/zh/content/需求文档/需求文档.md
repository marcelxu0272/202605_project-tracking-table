...
- [server/approval-engine.js](file://server/approval-engine.js)
...
- 新增数据库表结构：report_lines、report_line_data、report_line_pm_status、report_line_approvals
- 新增报告线服务与API：周期开启、列表查询、数据保存、审批操作、数据导出
- 新增前端组件：报告线列表、报告线详情、路由配置
...
- **新增** 数据库表结构：report_lines、report_line_data、report_line_pm_status、report_line_approvals
- **新增** 报告线服务与API：周期开启、列表查询、数据保存、审批操作、数据导出
- **新增** 前端组件：报告线列表、报告线详情、路由配置
...
- **新增** 报告线服务异常：检查 report_lines 表结构、数据保存接口、审批状态流转
- **新增** 前端组件问题：检查 ReportLineList 和 ReportLineDetail 组件的 API 调用

**更新** 新增了报告线管理、多角色协作、报告线服务、前端组件等故障排查指导。
...
REPORT_LINES ||--o{ REPORT_LINE_PM_STATUS : "PM状态"
REPORT_LINES ||--o{ REPORT_LINE_APPROVALS : "审批记录"
REPORT_LINES ||--o{ REPORT_LINE_DATA : "项目数据"
```
...
- [database-schema.sql:337-411](file://database-schema.sql#L337-L411)
...
- 风险评估：平台同步失败、导入合并冲突、审批流程阻塞、数据一致性风险、邮件提醒服务异常、J版归档失败、库存验证错误、预警忽略失效、报告线管理异常、多角色协作问题、报告线服务异常、前端组件问题
- 变更控制策略：变更写入审计日志、字段字典版本锁定、平台同步与导入合并规则、J 版归档后重置流程态、动态时间窗配置、手动永久忽略预警、Git-like数据流模型

**更新** 新增了报告线管理异常、多角色协作问题、报告线服务异常、前端组件问题等风险评估。
...
### 数据库表结构设计
- **report_lines**：报告线主表，存储按板块×周期创建的报告线，支持多级审批流程
- **report_line_data**：报告线项目数据表，存储报告线内各项目的字段数据与相对baseline的变更差异
- **report_line_pm_status**：PM提交状态表，记录报告线内各PM的提交状态
- **report_line_approvals**：审批记录表，记录报告线审批流程的每一步操作
...
### 审批引擎与状态机
- **状态转移规则**：定义了报告线的完整状态机，包括 open、reviewing_director、reviewing_leader、completed、rejected、closed 状态
- **审批链计算**：考虑自动跳过逻辑，支持多角色用户的审批流程
- **权限验证**：严格的权限控制，确保只有具备相应角色的用户才能执行特定操作

**新增** 审批引擎与状态机的详细说明。

**章节来源**
- [server/approval-engine.js:22-39](file://server/approval-engine.js#L22-L39)
- [server/approval-engine.js:145-189](file://server/approval-engine.js#L145-L189)
- [server/approval-engine.js:246-288](file://server/approval-engine.js#L246-L288)

### 报告线服务与API
- **周期开启**：为各板块创建新周期报告线，复制项目数据并初始化PM状态
- **列表查询**：按权限过滤查询报告线列表，支持多种筛选条件
- **数据保存**：保存填报数据并计算相对baseline的变更差异
- **审批操作**：支持PM提交、板块管理员提交审批、审批决策等操作
- **数据导出**：将报告线数据导出为Excel格式

**新增** 报告线服务与API的详细说明。

**章节来源**
- [server/report-line-service.js:47-102](file://server/report-line-service.js#L47-L102)
- [server/report-line-service.js:108-172](file://server/report-line-service.js#L108-L172)
- [server/report-line-service.js:252-280](file://server/report-line-service.js#L252-L280)
- [server/report-line-service.js:397-450](file://server/report-line-service.js#L397-L450)
- [server/report-line-service.js:652-706](file://server/report-line-service.js#L652-L706)

### 前端组件与路由
- **报告线列表组件**：实现筛选、分页、操作按钮等功能，支持手动发起填报
- **报告线详情组件**：继承项目编辑器，实现报告线专用的数据加载和操作
- **路由配置**：新增报告线相关路由，支持嵌套路由和参数传递
- **组件集成**：与Store模块集成，实现数据状态管理和API调用
...
- [js/router.js:7-69](file://js/router.js#L7-L69)
- [js/store.js:612-678](file://js/store.js#L612-L678)

### 报告线种子数据
- **模拟数据**：为报告线系统提供演示数据，包含上月和当月两种状态
- **数据结构**：包含板块、状态、PM状态、审批记录、项目数据等
- **用途**：用于前端演示和测试，支持数据导入和清理功能
