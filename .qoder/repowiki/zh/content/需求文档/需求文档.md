...
- 文档层：需求文档（产品版/开发版/新结构大纲）、设计文档（看板/列表/DESIGN）、字段字典（JSON/JS）
...
## 结论
本需求文档集合以“新结构大纲”为纲，结合“产品版/开发版”需求、设计规范与数据库模型，形成了覆盖角色权限、数据同步、填报交互、周期与锁定、变更追踪、审批与快照、系统管理、业务规则与补充确认的完整需求谱系。技术栈与开发规范明确了轻量化架构、CDN 依赖与前端组件使用方式，为项目实施提供了清晰的路线图。
...
### 用户故事、用例图与需求跟踪矩阵
- 用户故事：项目经理（填报产值/进度/WIP）、板块管理员（汇总核查/提交审批）、板块总监/群主（审批）、系统管理员（归档/配置/审计）
...
- 风险评估：平台同步失败、导入合并冲突、审批流程阻塞、数据一致性风险
- 变更控制策略：变更写入审计日志、字段字典版本锁定、平台同步与导入合并规则、J 版归档后重置流程态
