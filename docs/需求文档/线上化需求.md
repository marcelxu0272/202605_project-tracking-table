...
> **最后更新：** 2026-05-25（合同额 N/O/P：O 改系统取值、P 改公式 N+O；移除数据看板）
...
| 角色 | 定位 | 编辑权限 | 审批权限 | 说明 |
|---|---|---|---|---|
| **系统管理员** | 系统运维 | ✅ | △ | 负责系统配置、用户管理、权限分配、数据锁定/解锁、报表生成配置等；**填报表格**页可编辑全公司项目（含工程平台**引用列**覆盖，见 **§2.2.1**）；**唯一**可访问**审计日志**页；工具栏「**刷新数据**」（§2.2）、「**提交归档**」生成 **J版**（§2.15）。 |
| **经营管理（只读）** | 汇总查阅 | ❌ | ❌ | 原「财务审核」扩展为经营管理只读用户（财务、板块/群非审批领导、公司管理层）。按 **dataScope**（全公司 / 板块 / 项目群）过滤**填报表格**，**始终只读**；**无**审批流程、**无**审计日志、**无**管理设置。次月 1–3 日核查提醒横幅仍可见。 |
| **板块管理员** | 数据管理 | ✅ | ❌ | 负责板块内数据汇总校验、异常数据处理；可见填报、审批流程；工具栏「**刷新数据**」（§2.2、§2.11）；**不可**访问审计日志。 |
| **项目经理** | 数据填报 | ✅ | ❌ | 负责本人管辖项目的产值、进度、WIP 分析填报，Excel 导入/导出。 |
| **板块总监** | 审批决策 | ❌ | ✅ | 审批流程页查看本板块全部项目（**§2.14**）；**只读** Luckysheet；改数须 **驳回**。 |
...
| **group** | `groupRegistry[groupCode].sectors` 下全部板块 | 项目群非审批领导 |

- 用户对象：`role: executive_viewer` + `dataScope` + `sectorCode` / `groupCode`（见 **§2.18**）。
- **项目群**在管理设置维护 `meta.groupRegistry`（群编码 → 名称 + 板块列表）。
- 同一人兼「审批决策」与「经营管理查阅」：首版使用**两个账号**。
...
### 2.2 数据同步与刷新机制
- **滞后可接受：** 跟踪表全量项目范围来自**工程平台**（CRB 合同/项目 + 产值/开票/回款等），但表格**不实时**跟随平台变化。以 **`project_no` 为唯一索引**，项目只增不删；**显示值**以 Excel 导入/管理员修正为准，平台同步写入 **`_system_ref` 引用快照**（见 **§2.2.1**）。
- **批量刷新（已实现框架，开发期 stub）：**
  - **手动刷新（已实现）：** **系统管理员**与**板块管理员**在填报页点击「**刷新数据**」→ `POST /api/editor/refresh-data`：① 工程平台引用同步（新增项目、全部 `system_sync` 列 `_system_ref`、报告月 MI/MP）；② 重载 SQLite 中**最新项目 payload**（含各 PM 已提交/已保存数据、板块管理员改数、审批驳回后恢复的数据）。**不**覆盖未被管理员覆盖的系统引用显示值（§2.2.1）。
  - **自动同步：** 每日定时（默认本地 **02:00**）执行与手动刷新相同的平台引用同步；**达到月度锁定日**后暂停自动任务。
  - **合并规则（§2.2.1）：** 平台同步**仅**更新 `_system_ref`，**不**自动改写已有项目的显示字段；平台新增项目号 → INSERT（显示值=平台值，不设覆盖标记）。
...
#### 2.2.1 工程平台引用列（显示值 / 引用值 / 管理员覆盖）

| 维度 | 规则 |
|---|---|
| **引用列** | 全部 `system_sync`（**15 列**，含 **A 列新/旧项目**，见 **§2.2.2**）+ 报告月当月开票/回款（`mi_*` / `mp_*`）；数据源均为**工程平台** |
...
| **管理员覆盖** | 与 §2.2.1 一致：系统管理员可编辑 A 列显示值；审计 `system_ref_override`；清空可恢复引用值 |
...
- **与「新增项目行」高亮（§2.7）区分：** `_added_this_month` 表示相对 **baseline** 新出现的项目号；A 列「新/旧项目」表示**当年**业务分类，二者独立。

#### 2.2.3 合同额区（N / O / P，已实现）

| 列 | 字段 | 来源 | 规则 |
|---|---|---|---|
| **N** | 截止上一年底合同额 | `system_sync` | 每年 1 月 1 日（或跨年 rollover）从 CRB/历史库锁定上一年度期末合同额；前端只读。 |
| **O** | 当年调整值 | `system_sync` | **工程平台（CRB）同步**：本项目在**报告年度**内已完成的 **CRB3** 与 **CRB5** 合同额**之和**，表示当年合同变更增量；写入 `_system_ref.adj_value`；系统管理员可覆盖显示值（§2.2.1）。 |
...
- **月度锁定日（默认 25 日）：** `periodConfig.lockDay` — 报告月内当日 ≥ 锁定日时填报窗口关闭、数据冻结。除系统管理员外，任何人不可编辑（系统管理员仍受 **§2.8.5** 月度列时间窗约束）。
  - *管理员特权：* 系统管理员在锁定期可临时修改**允许编辑范围内**的字段（如 WIP 分析、未来月预测等），操作全程留痕；**不可**在线改写报告月当月及之前的开票/回款实际值列。
- **次月解禁日（默认 9 日）：** `periodConfig.unlockDay` — 业务规则上的常规解禁节点；配置项已在管理设置保存，**自动锁定判据见 §2.4.1**（当前实现以「报告月 + lockDay」为主，非报告月或未达锁定日默认为 `open`）。
- **管理设置（已实现）：** 系统管理员在「填报周期配置」修改报告月、提醒日、锁定日、解禁日并保存；「数据锁定控制」提供 **立即锁定 / 解除锁定**（写入人工覆盖，全程审计）。
- **数据初始化规则 (每月开启填报时自动执行)**：
  - **完成额（当月）**：报告月对应月度完成列（AV–BG 中当月格）自动重置为 `0`，等待 PM 手工填报。
  - **开票/回款（当月）**：报告月对应列由**工程平台**同步至 `_system_ref`；显示值默认保留 Excel 导入；PM 只读，系统管理员可覆盖显示值。
  - **历史保留**：报告月**之前**的月度完成/开票/回款均为只读历史数据。
  - **未来预测**：报告月**之后**的开票/回款列为预测值，可继承上一轮填报并允许编辑修改。
- **动态调整：** 上述三个时间节点（提醒日 / 锁定日 / 解禁日）在管理设置 **`periodConfig`** 中动态配置，以应对节假日或临时调整。

#### 2.4.1 锁定状态计算与刷新（已实现）

| 机制 | 行为 |
|---|---|
| **自动计算** | 库中**无** `meta.lockStatus` 时：自然月 = `reportingMonth` 且 **当日 ≥ `lockDay`** → `locked`，否则 → `open`。实现：`js/store.js` `calcLockStatus`、`server/db.js` `_calcLockStatus`。 |
| **人工覆盖** | 管理设置「立即锁定 / 解除锁定」写入 `meta.lockStatus`（`open` \| `locked`）。存在覆盖时**优先于**日期规则；**仅改 lockDay 不会自动解除**人工锁定，须点「解除锁定」或开发重置（§2.10）。 |
| **保存周期配置** | `Store.savePeriodConfig` → `PATCH /api/meta` 返回 bootstrap；前端 **`applyBootstrap` 立即刷新 `lockStatus`**，避免仍显示修改 lockDay 前的锁定态（如将锁定日 25 改为 27 后，25 日应立刻变为填报中）。 |
...
2. **板块管理员「提交审批」** → append **D 版**（本板块子集）；`reportingSubmitted = true`；驳回后可再次提交生成新版 D。
3. **总监 / 群主审批通过** → 仅更新 `approvalStatus`，**不写** Approve1/Approve2 快照。
4. **系统管理员「提交归档」** → append **J 版**（全量）；更新 `baselineVersion`；**可多次提交**（无 409 限制）。

- **版本对比：** 审批页可对 D/J 快照做 Diff（总监/群主审批页不提供，见 **§2.14**）。
- **填报页历史版本预览：** 工具栏选择 I/D/J 快照只读展示（见 **§2.12**）。
...
- **🆕 新增项目整行高亮（已实现）：** 对比 **`meta.baselineVersion`** 指向的快照（首期无 J 时为 **I 版**），当前库中**项目号不在 baseline 内**的行标记 `_added_this_month` / `_added_since_baseline`。
...
- **🎨 单元格底色叠色优先级（已实现）：** 同一格仅展示一种主底色，优先级从高到低：
  ...
  | 优先级 | 场景 | 底色 / 样式 |
  |---|---|---|
  | 1 | **预警** | 浅红底 `#fee2e2` + 红字 `#dc2626`（见 **§2.7.1**） |
  | 2 | 本月有变更 | 浅橙 `#ffedd5` + 深橙字 `#9a3412` + 左边框 `#ea580c` |
  | 3 | **系统引用已覆盖** | 浅蓝 `#e8f4fd` + 引用批注（§2.2.1） |
  | 4 | 当前可编辑列 | 浅黄 `#fefce8` |
...
- **提交阻断（已实现）：** 见上「提交阻断」；典型场景：对冲后仍无法满足、或 CRB 合同额下调后需人工调减历史月份完成额。
...
- **存量合同额（已实现，详见 §2.7.1）：** 填报开放期对 S < 0 自动对冲报告月完成额；**仅 PM/板块「提交」时**校验 S ≥ 0，编辑/保存过程不阻断。
...
### 2.11 项目经理提交与板块汇总（已实现）

项目经理的「**提交**」是面向板块管理员的内部汇报动作，**不等同于**板块「提交审批」（Draft）。**默认每个 PM 每个报告月仅可提交一次**；提交后 PM 锁定，**不可**再次编辑或重复提交；数据有问题由**板块管理员**在板块汇总表中直接修正。

#### 角色页面权限

> **说明：** 首版**不提供**独立「数据看板」页（KPI 卡片 / 图表汇总）；各角色在**填报表格**或**审批流程**中查阅与操作数据。公司级运营看板若需对接，另立项目，不在本系统范围内。

| 角色 | 可见页面 |
|---|---|
| **项目经理** | 填报表格 |
| **板块总监 / 项目群群主** | 审批流程 |
| **经营管理（只读）** | 填报表格（按 dataScope 过滤，只读） |
| **板块管理员** | 填报表格、审批流程 |
| **系统管理员** | 全部页面（**含唯一**审计日志） |

- **审计日志**仅 **system_admin** 可访问（`/audit` 路由与侧栏均限制）。
- 经营管理 / 项目经理 / 板块管理员 / 审批角色均**不可**进入审计页。
- 板块总监 / 项目群群主访问 `/editor`、`/audit`、`/admin` 时重定向至 **审批流程**（`/approval`）；经营管理可进入 `/editor`（只读）。
- 登录后默认进入：**填报角色** → `/editor`；**板块总监 / 项目群群主** → `/approval`。
- 填报表格按 **`pm_name`** 或 **dataScope** 过滤可见项目。
- 模拟项目经理账号：**何孝刚**（5 条项目）、**宋建生**（2 条项目），与数据库 `pm_name` 字段一致。

#### 提交状态机（当前实现）

```
[可编辑] ──提交（每月一次）──> [submitted: PM 锁定]
         │                              │
         │  数据即时写入 SQLite          │  板块管理员汇总表可见同一库数据
         │  （无需「确认接收」）          │  「查看变更」对比基准 vs 提交快照
         │                              │
         │                    板块管理员修正 / 汇总
         │                              │
         └──────── 不可再次提交 ────────┘
...
- **Excel 导入：** PM 可在填报页「上传导入」，仅合并本人项目可编辑列（见 **§2.12.3**）；**已提交后** PM 锁定，不可导入。
- **提交前兜底（已实现）：**
...
- **注意：** 灰色/公式列（`auto_calc`、`system_sync`）不可由 PM 保存；可入库的手工列见 **§2.8.5**。
...
| **右侧** | **刷新数据**（系统管理员 / 板块管理员）· **导出 Excel · 上传导入**（一组）｜ **保存 · 提交归档 · 提交/提交审批**（一组，竖线分隔） |
...
- **布局（Header 以下滚动区）：**
  ...
  2. **跟踪信息填报：** 区块标题右侧 **`?` Tooltip** 说明：浅黄色底色 = 当前角色可编辑字段（与 Luckysheet **§2.7** 一致）；未产生 WIP 时分区自动折叠及无需填写 WIP 相关内容的规则；
  ...
  5. **辅助区（「项目工时与成本」，P2 已实现）：** 与参考区相同 **`el-collapse` + `drawer-collapse-title`（图标 + 名称）** 结构；含 **工时数据**、**成本数据** 两个折叠项（默认收起）；均为**只读参考**，不参与填报与审批。详见 **§2.12.9**、**§2.12.10**。
...
- **保存：** Drawer 内批量 diff → 单次 `PUT` + 公式重算 → `syncLuckysheetProjectRowValues` 就地更新该行单元格值与高亮（不整表 `refreshLuckysheet`）；完成额仅校验有效数字；**S 列约束仅在提交时校验**（§2.7.1）；切换项目不自动保存未提交编辑。
...
- **实现：** `js/project-drawer-layout.js`、`js/project-alerts.js`（**§2.12.11**、完成额填报参考）、`js/stock-validation.js`（完成额非负 + S 列）、`js/components/ProjectDetailDrawer.js`、`js/components/ProjectTimesheetAux.js`（**§2.12.9**）、`js/components/ProjectCostAux.js`（**§2.12.10**）、`css/style.css`（Drawer 样式与 `--color-editable-cell-bg`）、`css/element-theme.css`；`ProjectEditor.js` 提供 `navigateProjectDrawer`；`ApprovalReviewSheet` 继承 `ProjectEditorView`，审批待办节点同样可用 F 列入口。
...
| **存量合同额为负** | S 列（存量合同额 = P − U）**< 0** | 公式重算后的项目数据 | 是（**仅提交时**校验 S ≥ 0；开放期自动对冲报告月完成额，见 **§2.7.1**） |
...
### 2.13 I/J baseline 快照与新增项目演示（已实现）
...
- **视觉：** 保留 **§2.7** 新增行底色、可编辑列浅黄（本角色无可用可编辑列时均为只读态）、变更格高亮与批注图例（可关闭）。
...
| **当年调整值（O）** | **CRB (合同系统)** | 本项目**报告年度**内已完成的 **CRB3 + CRB5** 合同额总和；系统同步至 `_system_ref`，见 **§2.2.3**。 |
...
- **存量合同额约束（已实现）：** 填报开放期若 **S < 0** 自动将报告月完成额调减（可为负）使 S 归零；**提交时**须全部 **S ≥ 0**（§2.7.1）。锁定期内允许数据中暂存 S < 0，不在保存时阻断。
...
| # | 补充项 | 状态 | 说明 |
|---|---|---|---|
| 1 | **单元格底色区分（数据源可视化）** | ✅ 已实现 | 默认斑马纹/只读灰；**管理员覆盖**工程平台引用列后浅蓝 `#e8f4fd` + 引用批注（§2.2.1）。不再默认整列浅蓝。 |
| 2 | **项目详情 Drawer / 枚举·长文本编辑** | ✅ 已实现（P0/P1+） | F 列打开 Drawer（§2.12.8）：Header + 项目切换；浅黄可编辑态；报告月完成额 Popover 参考；金额千分位；WIP 为 0 自动折叠；保存回写 Sheet。 |
...
| 8 | **存量指标 R/S 预警与校验** | ✅ 已实现 | R/S 预警筛选；开放期 S<0 自动对冲报告月完成额；**仅提交时**校验 S≥0。详见 **§2.7.1**、`js/stock-validation.js`。 |
| 9 | **表格工具栏与紧凑列** | ✅ 已实现 | 关闭 Luckysheet 内置顶栏；保留 **公式/编辑栏**（`sheetFormulaBar`）；`.sheet-toolbar` 承载项目筛选 + 紧凑列。详见 **§2.8.6**、**§2.12.1**。 |
| 10 | **Drawer 工时辅助统计** | ✅ 已实现 | 折叠项「工时数据」；专业×月 / 板块×月、工时/成本切换、明细与下钻；**无数据时仅「暂无已审工时记录」**。详见 **§2.12.9**。 |
| 11 | **Drawer 成本中心辅助统计** | ✅ 已实现 | 折叠项「成本数据」；成本项×月矩阵、明细与下钻；**无数据时仅「暂无成本记录」**。详见 **§2.12.10**（辅助参考，非成本法产值）。 |
...
| 15 | **总监/群主审批只读** | ✅ 已实现 | 审批页 Luckysheet 恒只读；需改数须驳回，由板块管理员编辑（§2.14、`field-config.js`）。 |
| 16 | **Luckysheet 公式栏** | ✅ 已实现 | `sheetFormulaBar: true`，还原单元格内容编辑栏；原生工具栏仍关闭（§2.8.6）。 |
| 17 | **保存周期配置刷新锁定态** | ✅ 已实现 | 保存 `periodConfig` 后 `applyBootstrap` 同步 `lockStatus`；横幅锁定日读配置；人工锁定与日期规则见 **§2.4.1**。 |
...
| 字段来源 | 可编辑角色 | 说明 |
|---|---|---|
| **工程平台引用列**（`system_sync` + 报告月 MI/MP，**含 A 列新/旧项目**） | 默认全部 R；**系统管理员 W**（覆盖显示值） | 同步写 `_system_ref`；覆盖写 `_system_override`（§2.2.1、§2.2.2） |
| **auto_calc**（公式计算） | 全部 R | 不可编辑 |
| **manual_input（通用）** | 系统管理员 W · 板块管理员 W · 项目经理 W | 总监/群主/经营管理 R |
| **manual_input（报告月后开票/回款预测）** | 同上 | 报告月当月 MI/MP 归入引用列 |
| **manual_input（WIP 分析）** | 系统管理员 W · 板块管理员 W · 项目经理 W | 经营管理/总监/群主 R |
| **项目号 F** | 全部 R | 运行期不可编辑 |

### 2.18 用户与权限配置（管理设置）

- **持久化：** `meta.users`、`meta.groupRegistry`；API `GET/PATCH /api/admin/users`。
- **用户管理：** 姓名、角色、`executive_viewer` 的 dataScope / sectorCode / groupCode、启用/停用。
- **项目群配置：** 群编码、名称、所含板块（多选 `sectorRegistry`）。
- **登录页：** 从 bootstrap 用户列表按角色选人；演示账号覆盖 company / sector / group 三档经营管理范围。
...
| **新、旧项目（A 列）** | 区分历史遗留 vs 新增 | **系统引用列**（§2.2.2）：当年平台新增默认为「新项目」；跨报告年初全部 rollover 为「旧项目」；系统管理员可覆盖显示值。与 §2.7「新增项目行」高亮（上月快照对比）**不是同一概念** |
| **R 列 / S 列** | 总合同额-开票 / 总合同额-完成 | 已更名为 **存量开票额（R）** / **存量合同额（S）**；分区名 **存量指标**（§2.7.1） |
| **当年调整值（O）/ 总合同额（P）** | O=P−N；P 直取 CRB | **O** 为 CRB 同步（当年 CRB3+CRB5 完成额之和）；**P=N+O** 公式计算（§2.2.3） |