...
> **最后更新：** 2026-06-08（报告线填报页导入：`ProjectEditor` 上传按钮改由 `canImport` 控制；`ReportLineDetail.canImport` 对 PM / 板块管理员开放；覆盖 `onImportFileChange` 写入 `saveReportLineData` 并尊重可见范围与分发列）
...
| 2026-06-08 | 恢复报告线模式下板块管理员 PM 提交情况：`ReportLineDetail.submittedPmSubmissions` 改读 `reportLine.pmStatuses` 且展示全部 PM 状态；`ProjectEditor` 提供标题/提示/meta 格式化钩子；`showPmDiff/renderPmDiff` 改用报告线项目和 `baseline_version` 快照 | 7.5 |
...
| **右侧** | **刷新数据**（系统管理员 / 板块管理员）· **导出 Excel · 清零当月完成额 · 上传导入**（一组）｜ **保存 · 提交归档**（仅锁定期可用） **· 提交/提交审批**（一组，竖线分隔） |
...
- 填报页底部显示「**本月已提交 PM**」面板（本板块内按平台项目经理绑定关系归属的 PM，`status === 'submitted'` 或历史 `received`），含「**查看变更**」；标题栏提供**收起 / 展开**（默认展开，对齐系统管理员「各板块进度」底栏）。
- **「查看变更」：** `baselineVersion` 快照 vs 当前库中该 PM 平台绑定项目子集（`manual_input` 列 diff）。
...
- 「查看变更」对比的是 **PM 平台绑定项目子集的本轮改动**，不是「提交版 vs 管理员整张表」。
...
> **需求背景：** 公司各个执行单位（板块/中心）的审批流程**并行**、互不影响；系统管理员需在审批页纵览各板块三步进度，在填报页纵览状态并按板块查看变更，在**锁定期**内提交 **J版** 全公司归档。
...
- **系统管理员：** 审批页见上节；**J版** 仅在填报页工具栏「**提交归档**」（**仅锁定期**可用）；成功后底栏 Tag「公司已归档 J版」。
...
| `js/views/ProjectEditor.js` | 工具栏「提交归档」、`canShowArchiveButton`、`canSubmitArchive`（锁定期校验）、版本下拉 `v-if="isSystemAdmin"`、`editorSnapshotOptions`（J 版过滤）、`isViewingJSnapshot`、`handleDownloadSnapshot`、`downloadSnapshotLoading` |
...
| `server/report-line-service.js` | 新增 `normalizeDistributedColumns`（强制 E/F/G，去重排序）；`forkPeriod` 接收 `options.distributedColumns`，写入 DB；`getReportLineDetail`/`getReportLines` 返回 `distributed_columns`（JSON 解析）；`exportReportLine` 按分发列过滤导出字段（内嵌 `COL_TO_KEY`）；新增 `resolveApprovalStaffForSector`、`getForkPreview` |
| `server/index.js` | fork-period 接口解构 `distributedColumns` 并透传；新增 `GET /api/report-lines/fork-preview`；报告线导出接口接收 `role/pmName/sectorCode` 查询参数 |
| `js/store.js` | `forkReportPeriod(period, distributedColumns)` 更新签名，支持透传列配置；新增 `fetchForkPreview()`；`getVisibleOperations()` 对历史月份统一返回 `view/export` |
| `js/views/ReportLineList.js` | 弹窗新增「分发列配置」区域：`forkDistMode`（all/custom）、`forkSelectedCols`；computed `forkColumnGroups`/`forkSelectedCount`；methods `isMandatoryForkCol`/`toggleForkCol`/`selectAllForkCols`/`clearForkCols`；`confirmFork` 传 `distributedCols`；导出时透传当前用户身份参数 |
| `js/views/ProjectEditor.js` | 模板增加报告线扩展钩子（`rlContextBar`、`canShowEditorSaveGroup` 等） |
| `js/views/ReportLineDetail.js` | 新增 `distributedColumnSet` computed（列字母→colIdx Set）；覆盖 `buildLuckysheetColhidden`（叠加未分发列隐藏）；`_flushRlLuckysheet` 跳过隐藏列（防数据覆盖）；`scopedProjects` 按角色过滤，PM 仅看本人项目 |
...
| `handleRlSubmit()` | PM 调 `Store.pmSubmitReportLine`，板块管理员调 `Store.submitReportLineApproval` |
