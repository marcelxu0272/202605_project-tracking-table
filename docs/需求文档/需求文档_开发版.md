...
| 日期 | 更新内容 | 涉及章节 |
|---|---|---|
| 2026-08-05 | Luckysheet 补丁：`SUBTOTAL` 识别筛选隐藏行并重算小计；方向键/Enter 跳过隐藏行；`forceCalculation`；`js/luckysheet-subtotal-patch.js`、`js/luckysheet-nav-skip-hidden.js` | 3.3、3.4 |
| 2026-08-03 | `ProjectDetailDrawer` 左侧「项目基准」改用 `baselineIdentityFields`（F/G/H/E/D/K + 项目周期）；移除标签区；不再展示 I/J | 3.7 |
| 2026-08-03 | 报告线新增 `finalizing`（核对归档中）：`reviewApproval`/`autoCompletePeriod` 目标态改为 finalizing 并全量同步主表；`PUT/POST /api/projects` 保存后同步 finalizing 线；`POST /api/company/archive` 封账为 completed；前端归档前 `flushLuckysheetToStore`；`test/report-line-finalizing.test.js` | 3.5、4.1、7.5 |
| 2026-07-31 | `ProjectDetailDrawer.headerTagFields` 去掉 A 列，左侧标签仅 I/J/K | 3.7 |
| 2026-07-31 | `computeDrawerMetrics` KPI：`开票/回款`→`完成开票额/完成回款额`，新增 `prevYear`；`ProjectDetailDrawer` KPI 增加「截止上年」行，矩阵表头同步；更新测试 | 3.7 |
| 2026-07-31 | 抽屉布局：`DRAWER_TAB_CONFIG` 移除 `status`/`extended`；新增 `LEFT_PROGRESS_COLS=['M']` 与 `progressFields`；左侧卡片改名「WIP与应收账款」，下方增加项目实施进展编辑区；`ProjectDetailDrawer` 去掉对应 Tab | 3.7 |
| 2026-07-31 | `BASELINE_METRIC_COLS` 改为 `AL/AJ`（去掉 S/R 存量合同额/存量开票额）；更新 `test/project-drawer-layout.test.js` | 3.7 |
| 2026-08-06 | 新增三类开票/回款预警（`invoice_exceeds_contract` / `payment_exceeds_contract` / `payment_exceeds_invoice`）；Drawer 原「存量开票额为负」改为「累计开票超总合同额」；`StockValidation` 新增检测函数；`test/payment-alert-validation.test.js` | 3.7.2、7.3、7.4 |
| 2026-07-29 | `completion-negative-validation` 拆分 `save`/`submit` 模式：保存口仅禁未来月负值，提交口额外要求当前月负值备注；前端 `shouldGuideNegativeRemark` 仅在「本次改负」引导；`ProjectEditor`/`ReportLineDetail` 对齐；测试覆盖分层校验 | 3.7、5.6、7.5、8.3 |
| 2026-07-28 | 新增字段 `CF -> completion_remark`（字段字典 84 列）；`ProjectDetailDrawer` 经营预测 Tab 月度矩阵下新增备注输入区；当前月完成额负值时失焦自动拉起抽屉并聚焦备注；`stock-validation` 增加“未来月完成额不可负、当前月负值备注必填”规则；服务端新增 `server/completion-negative-validation.js` 并接入主项目保存与报告线保存/提交；新增测试 `test/negative-completion-remark-validation.test.js` | 3.7、5.6、7.5、8.3 |
| 2026-07-28 | 预警去掉已消除：sync 删除条件消失行；筛选默认全部；`deleteAlert` / `deleteResolvedAlerts` | 7.4 |
| 2026-07-28 | 预警忽略支持撤回：`undismissAlert` / `undismissAlertById` / `POST .../undismiss`；`AlertsDrawer`「撤回」；`test/alert-undismiss.test.js` | 7.4 |
| 2026-07-28 | `alert-demo-seed` 新增 `multi_alert`=`B24264-0039`（宋建生）同项目多预警；工时日期按报告月动态生成 | 7.3、7.4 |
| 2026-07-28 | `AlertsDrawer` 卡片去掉「首次检测」展示与 `formatTime`；库表仍保留 `first_detected_at` | 7.4 |
| 2026-07-28 | 系统与功能名称：登录/侧栏「项目执行跟踪」；`AppLayout`/`router`「填报与审批」「查看数据」；导出文件名 `查看数据_*` / `项目执行跟踪_*`；提示与返回按钮文案同步 | 1.5、3.5、7.5 |
| 2026-07-28 | `BASELINE_METRIC_COLS` 改为 `S/R/AL/AJ`（去掉 AC/AF）；`RATE_CARD_METRIC_COLS` 增补 AC/AF，避免延伸数据重复；更新 `test/project-drawer-layout.test.js` | 3.7 |
| 2026-07-27 | `computeDrawerMetrics` 新增 `progressCards`（合同完成率/开票进度/回款进度）及 `invoiceRate`/`paymentRate`；`ProjectDetailDrawer` 左侧用 `v-for` 渲染三张 `.drawer-baseline-rate-card`；始累完成文案去掉「项目」；更新 `test/project-drawer-layout.test.js` | 3.7 |
| 2026-07-27 | 填报管理列表双状态：`server/report-line-my-status.js` 提供 `resolveMyStatus`；`getReportLines` 返回 `my_status`（PM 批量查 `report_line_pm_status`）；`ReportLineList` 列「我的状态」+「报告线状态」，筛选文案同步；`test/report-line-my-status.test.js` | 7.5 |
| 2026-07-27 | 正式环境目标模型调整为 `formula`、`manual_maintained`、`system_direct`、`system_adjusted` 四类字段；删除 Demo 的手动刷新按钮、Store 方法及 `/api/editor/refresh-data`、`/api/admin/sync-platform-data`；正式事件接口、系统值/调整值存储及冻结期积压处理仅定义契约，本次不实现 | 0.4、1.1、1.3、2.1–2.6、3.4、6.2 |
| 2026-07-16 | 抽屉左侧指标：`BASELINE_METRIC_COLS` 改为 `S/R/AC/AF/AL/AJ`（移除 AP 期初 WIP）；新增 `BASELINE_METRIC_LABELS` 覆盖抽屉展示名（始累开票、始累回款、WIP、应收账款） | 3.7 |
| 2026-07-16 | `project-drawer-layout.js` 修正 `computeDrawerMetrics` 的 KPI 剩余口径：完成合同额/开票/回款分别改为 `total_contract - prev_year_completion/invoice/payment - ytd - forecast`；更新 `test/project-drawer-layout.test.js` 断言 | 3.7 |
| 2026-07-14 | 左侧完成率卡：`.drawer-baseline-rate-*` 布局为左百分比、右「项目始累完成合同额」「总合同额」；`BASELINE_METRIC_COLS` 去掉 P/U；新增 `RATE_CARD_METRIC_COLS` 并从延伸数据去重；项目基准周期拆为起始/结束两行 | 3.7 |
| 2026-07-14 | 抽屉分区一律默认展开：`timesheet_cost` 去掉 `el-collapse` 改为常显 section；`expandAllCollapseSections` 展开全部 `primaryActive`/`readonlyActive`/`auxActive`；WIP=0 仅提示不自动收起；`buildTabLayout` 透传 `editableSections`/`readonlySections` | 3.7 |
| 2026-07-14 | 项目详情抽屉右下角「保存」始终展示：`showSave` 恒 true，`saveDisabled`/`handleSave` 只读时禁用；产品版同步「底栏操作」 | 3.7 |
| 2026-07-14 | 工时/成本独立 Tab：`ProjectDetailDrawer` 新增 `timesheet_cost` Tab「工时与成本」；从延伸数据移除；`auxActive` 默认 `['工时数据','成本数据']` 展开 | 3.7 |
| 2026-07-14 | 经营预测 Tab 去掉 `drawer-forecast-banner` 与「年度口径明细」`forecastSummaryFields`；仅保留 KPI 行 + 月度矩阵 | 3.7 |
| 2026-07-14 | 项目详情抽屉经营预测文案与矩阵样式：`project-drawer-layout` KPI `产值`→`完成合同额`；`ProjectDetailDrawer` 标题「月度填报」、表头与图例同步；`matrixCellBadge` 报告月完成合同额「当月」/未来「预测」；报告月开票回款保持只读；`.drawer-tab-panel--forecast` + `.drawer-month-matrix*` 加高占满右侧 | 3.7 |
| 2026-07-14 | 填报管理列表顶部截止提醒：`ReportLineList` 新增 `deadlineReminder` computed，读取 `Store.reportingMonth` / `periodConfig.deadlineDay` 展示「每月 N 日」；`style.css` 新增 `.rl-deadline-banner*`；`store.js` `DEFAULT_CONFIG` 补齐 `deadlineDay: 25` | 7.5 |
| 2026-07-13 | 项目追踪表导出改为 Luckysheet 视图 WYSIWYG：`luckysheet-xlsx-export.js` 从 `file.data` 导出公式/样式/合并；`ProjectEditor` 改调 `exportCurrentLuckysheetView`；CDN 换 `xlsx-js-style`；详见 `docs/设计文档/LUCKYSHEET_XLSX_EXPORT.md` | 3.5 |
...
| **系统直取字段** | 业务系统是唯一权威来源，业务事件写入后直接更新，任何人不可编辑 |
| **系统调整字段** | `display_value = system_value + adjustment_value`；人工修正只改变调整值 |
| **业务事件更新** | 源业务功能完成合同变更、开票、回款等动作后主动发送的字段更新；不依赖人工刷新或每日轮询 |
...
flowchart TB
    ...
    subgraph 项目追踪表系统
        ...
        subgraph 后端["后端 (Node.js + Express)"]
            API["REST API"]
            SYNC["Demo 平台同步桩<br/>platform-sync.js"]
            EVENT["正式环境事件接收端<br/>待重构"]
...
| 角色 | 定位 | 编辑权限 | 审批权限 | 说明 |
|---|---|---|---|---|
| **系统管理员** | 系统运维 | ✅ | △ | 负责系统配置、审批人员配置、数据锁定/解锁、报表生成配置等；可按字段权限维护人工维护字段和系统调整字段的 `adjustment_value`，不可修改公式字段或 `system_value`；**唯一**可访问审计日志页；工具栏「提交归档」生成 J版（仅锁定期可用）；版本下拉仅系统管理员可见。 |
...
| **板块管理员** | 数据管理 | ✅ | ❌ | 负责板块内数据汇总校验、异常数据处理；可见填报、审批流程；进入填报/报告线表单时加载最新可见范围数据；**不可**访问审计日志。 |
...
正式环境字段来源模型需从当前 Demo 的 `source_type: auto_calc | manual_input | system_sync` 重构为四类。该重构及数据迁移不在本次实现范围；本次只移除人工同步入口并固化目标契约。

| 字段类别 | 可编辑角色 | 说明 |
|---|---|---|
| **`formula` 公式字段** | 全部 R | 固定公式计算；任何角色不可修改公式或结果 |
| **`manual_maintained` 人工维护字段** | 按字段配置 W | 外部平台不再更新；适用于行业性质、客户性质等历史数据缺失且低频变化字段 |
| **`system_direct` 系统直取字段** | 全部 R | 事件更新 `system_value` 并直接作为显示值；适用于项目经理、开始/结束日期等 |
| **`system_adjusted.system_value`** | 全部 R | 源业务事件写入，任何角色不可修改 |
| **`system_adjusted.adjustment_value`** | 按字段配置 W | 用户录入目标显示值时反算 `target - system_value`；调整原因必留痕 |
| **`system_adjusted.display_value`** | 派生 R | `system_value + adjustment_value`；公式、预警、导出统一使用该值 |
| **月度预测/WIP 等人工字段** | 沿用现有角色与月份窗口 | 在目标模型中归入 `manual_maintained` |
| **项目号 F** | 全部 R | 运行期不可编辑 |
...
- **正式环境目标（待重构，本文定义契约）：** 完成初始化后，合同变更、开票、回款、项目经理或项目周期变更等源业务功能在动作成功后产生业务事件，由追踪表事件接收端更新对应项目字段。
- **无变更不更新：** 页面加载、登录、保存、归档均不主动向源系统取数；没有新业务事件时，系统字段不变化。
- **无人工触发：** 本次已删除 `ProjectEditor` 的「刷新数据」按钮、`Store.refreshEditorData` / `Store.syncPlatformData`，以及 `POST /api/editor/refresh-data`、`POST /api/admin/sync-platform-data`。所有角色只读取当前已入库状态。
- **Demo 边界：** `server/platform-sync.js`、stub 与现有定时调度暂保留作为原型数据演示基础，不代表正式环境方案；本次不实现事件总线、Webhook、正式源系统 API 或四类字段的新存储结构。
- **项目准入：** CRB3、EPC 执行拆分、提前申报产值审批通过等业务事件可触发项目 UPSERT；以 `project_no` 定位项目，以 `source_system + event_id` 幂等。
- **局部更新：** 事件只携带并更新业务动作负责的字段，然后重算受影响的公式依赖，不允许用不完整事件覆盖整条项目 payload。
- **审计：** 正式事件需记录 `source_system`、`event_id`、`event_type`、`business_no`、`occurred_at`、`received_at`、字段差异及处理结果。

#### 数据同步流程图

```mermaid
flowchart LR
    SOURCE["源业务功能<br/>合同变更/开票/回款/项目信息"] --> EVENT["业务事件"]
    EVENT --> IDEMPOTENT{"source_system + event_id<br/>是否已处理？"}
    IDEMPOTENT -->|是| DONE["返回已处理"]
    IDEMPOTENT -->|否| TYPE{"field_value_mode"}
    TYPE --> DIRECT["system_direct<br/>写 system_value/display_value"]
    TYPE --> ADJUSTED["system_adjusted<br/>写 system_value<br/>保留 adjustment_value"]
    ADJUSTED --> DISPLAY["display_value =<br/>system_value + adjustment_value"]
    DIRECT --> FORMULA["按依赖重算公式"]
    DISPLAY --> FORMULA
    FORMULA --> DB[("项目字段存储")]
    DB --> AUDIT["事件与字段差异审计"]
```

### 2.2 四类字段存储契约（正式环境目标，待实现）

| `field_value_mode` | 建议存储 | 写入者 | 读取口径 |
|---|---|---|---|
| `formula` | 公式定义 + 可重建结果 | 公式引擎 | 公式结果 |
| `manual_maintained` | `manual_value` | 获授权用户 | `manual_value` |
| `system_direct` | `system_value`、源事件元数据 | 业务事件处理器 | `system_value` |
| `system_adjusted` | `system_value`、`adjustment_value`、源事件元数据、调整审计 | 事件处理器 + 获授权用户 | `system_value + adjustment_value` |

#### 2.2.1 系统调整字段写入规则

1. 事件处理器只允许写 `system_value`，不得清空或重写 `adjustment_value`。
2. 用户在界面输入目标显示值 `target_display_value` 时，服务端计算并保存 `adjustment_value = target_display_value - system_value`；客户端不得直接提交新的 `system_value`。
3. 恢复系统口径等价于将 `adjustment_value` 置零，不修改 `system_value`。
4. 公式、预警、快照、审批、导出统一读取派生 `display_value`；审计详情可同时展示三种值。
5. 调整数值请求需携带原因；审计至少保存操作人、字段、旧/新调整值、操作前后显示值与时间。
6. 金额计算使用统一精度与舍入策略，避免前端浮点反算造成分差。
...
{
  "sourceSystem": "crb",
  "eventId": "CRB3-20260727-0001",
  "eventType": "contract_amount_changed",
  "businessNo": "CRB3-0001",
  "projectNo": "P26001",
  "occurredAt": "2026-07-27T10:30:00+08:00",
  "fields": {
    "adj_value": 120000
  }
}
...
事件接口必须处理重复、乱序和冻结期积压。建议为每个源对象保留业务版本号或源更新时间，旧事件不得覆盖新事件。

### 2.3 A 列「新/旧项目」（目标：`system_direct`）

| 维度 | 规则 |
|---|---|
| **来源** | 目标模型为 `system_direct`；不再由 `FormulaEngine` 按合同签署年公式计算，任何人不可编辑 |
| **系统判定** | 项目准入事件首次建项时写入 `_new_existing_year = 报告年` 并显示「新项目」 |
| **已有项目** | `_new_existing_year === 报告年` →「新项目」，否则「旧项目」 |
| **年末 rollover** | 新报告年度开启事件触发重新分类；不得依赖每日同步任务顺便检测 |
| **初始化** | 导入值用于建立首个分类年度；正式切换后不接受覆盖 |

- **Demo 现状：** `js/new-existing-ref.js`、`server/platform-sync.js`、`server/xlsx-seed.js` 仍使用 `_system_ref/_system_override` 原型结构；正式重构时迁移到上述目标模型，本次不改。
...
| **O** | 当年调整值 | 目标 `system_adjusted` | CRB3/CRB5 完成事件更新报告年度合同变更额 `system_value`；人工修正写 `adjustment_value`；O 显示值为两者之和。 |
| **P** | 总合同额 | `auto_calc` | **`P = N + O`**；由 `FormulaEngine` 每次重算；前端不可编辑；下游 R/S/Q 等公式均以 P 为输入。 |

- **取值说明（O 列）：** CRB3、CRB5 为合同变更流程类型；流程完成事件按项目号更新本年度两类变更合同金额。为避免术语冲突，开发命名应区分“当年合同变更额”和通用 `adjustment_value`。
- **与旧逻辑差异：** 不再由 `O = P − N` 反推；**P 不再直接从 CRB 取 CRB1/CRB3 单值**，而由基准 N 与当年增量 O 合成。
- **初始化：** 若只能取得 O 显示值，须由业务确认系统值/调整值拆分后再迁移；导入后 `FormulaEngine` 重算 P。
- **Demo 现状：** `fields.json`、`js/formula-engine.js`、`server/platform-sync.js` 仍保留现有结构；四类字段迁移本次不实现。

### 2.5 锁定期管理

- `lockStatus` 继续控制人工编辑权限；正式环境另建 `data_update_state` 控制业务事件写入，禁止混用。
- 次月 1 日 00:00 起进入 `frozen`，事件只入积压队列，不更新已有项目系统值、不新增项目，公式有效月份保持待归档月。
- 次月自动归档恢复日（默认 8 日，可配置）依次执行：阻止新保存 → 创建唯一月结 J 版 → 切换有效计算月份 → 顺序处理积压事件 → 进入 `active`。
- 建议状态：`active | frozen | archiving | replaying | failed`。归档失败保持 `frozen/failed`；不得先恢复事件写入。
- 正式实现须提供任务幂等、服务启动补跑、积压事件顺序、失败告警和管理员只读状态查看；不得恢复人工同步接口。
...
- **工具栏视图筛选：** 「全部 / 有变化」位于 Luckysheet 上方**表格工具栏**（`.sheet-toolbar`，见 **§3.4.2**）；与「填报视图」切换无关。
- **Excel 导入/导出：**
  - 支持下载模板（包含已锁定的基础数据列）。
...
| 行号 | 用途 |
|---|---|
| 0 | **小计**（Subtotal）：金额列使用 `SUBTOTAL(9, 数据区)`；经 `luckysheet-subtotal-patch.js` 修补后排除筛选隐藏行并在筛选确认后重算 |
| 1 | **合计**（Total）：金额列使用 `SUM(数据区)`（筛选后不变）；标签文案显示在 **G 列**（「小计 Subtotal」「合计 Total」） |
| 2 | 大类 / 分区标题行（按 `section` 合并单元格） |
| 3 | 字段中文标题行 |
| 4+ | 项目数据行（顺序与筛选结果一致） |

小计/合计置于大分类之上，**默认筛选**仅覆盖「字段标题行 + 数据行」，避免汇总行被筛掉。方向键导航见 `js/luckysheet-nav-skip-hidden.js`（跳过 `filter.rowhidden` / `config.rowhidden`）。
...
- **公式链：** 创建 sheet 时预构建二维 `data` 矩阵并配置 `calcChain`（sheet `index` 为数字 `0`；先数据行公式，再小计/合计行），避免初始化时 `mergeCalculation` 访问空行报错；`forceCalculation: true`；加载后触发公式重算，保证 SUBTOTAL/SUM 与 `auto_calc` 正确显示。
- **SUBTOTAL 筛选补丁：** `js/luckysheet-subtotal-patch.js` 改写 `luckysheet_function.SUBTOTAL`，按 `sheet.filter[*].rowhidden` 过滤后再聚合；筛选菜单「确认/清除筛选」及 `updated`（`f`/`fsc`/`fsr`/`shr` 等）触发 `refreshFormula`。`ProjectEditor.initLuckysheet` 的 `workbookCreateAfter` / `updated` 接入；审批页 / 报告线详情继承同一套。
...
| **右侧** | **导出 Excel · 清零当月完成额 · 上传导入**（一组）｜ **保存 · 提交归档**（仅锁定期可用） **· 提交/提交审批**（一组，竖线分隔）；所有角色均无人工刷新/同步入口 |
...
| **存在人工调整值** | `system_adjusted.adjustment_value !== 0`；正式环境目标，Demo 暂沿用旧覆盖样式 | 14×14 浅蓝纯色块 `#e8f4fd` |
...
- **单元格右键：** 仅保留**复制**（`copy: true`）；关闭筛选选区（`filter: false`）及粘贴/插删行列等；行/列标右键关闭插删行列与改宽高。表头默认筛选区仍通过 `setRangeFilter` 开启。
...
- **筛选后导航：** `js/luckysheet-nav-skip-hidden.js`（`index.html` 于 Luckysheet CDN 后加载）捕获方向键与 Enter，跳过筛选隐藏行及紧凑列隐藏列；审批页与报告线详情共用。
...
- **提交归档（系统管理员，已实现）：** 与「保存」同组，按钮文案「**提交归档**」；仅 `role === system_admin` 且非快照预览时显示（`canShowArchiveButton`）；**仅锁定期可用**（`canSubmitArchive` 额外要求 `lockStatus === 'locked'`，开放期按钮 disabled 并 tooltip 提示「仅在锁定期可提交归档」）；`handleSubmitArchive` 先 `flushLuckysheetToStore()` 自动保存，再 `Store.queryReportLines({ status:'finalizing', period })` 生成确认文案，确认后 `Store.archiveCompany()` → `POST /api/company/archive` 生成 **J版**、重置流程态，并将当前报告月 `finalizing` 报告线封账为 `completed`（响应含 `finalizedReportLines`）；**后端**同样校验 `dbm.getEffectiveLockStatus(db) === 'locked'`，否则返回 403；**不**要求全部板块 Approve2；**不**自动改 `lockStatus`。
- **主表 → 核对归档中同步（已实现）：** `PUT /api/projects/:projectNo` 与 `POST /api/projects` 在写库后调用 `reportLineService.syncProjectToFinalizingReportLines` / `syncAllProjectsToFinalizingReportLines`：仅当前 `reportingMonth` 且 `status=finalizing` 的报告线；按 `project_no` 覆盖已有 `report_line_data`（不插入新项目、不删除）；`completed` 线跳过。
...
- **实现：** `ProjectEditor.exportCurrentLuckysheetView()` → `LuckysheetXlsxExport.exportSheetToXlsx()`（`js/luckysheet-xlsx-export.js`）；读取当前 Luckysheet `file.data` + `config.merge/columnlen`，**非** `XlsxImporter.exportToXlsx` 重拼表。
- **导出范围：** 当前 Luckysheet 视图项目（`filteredProjects` + 视图筛选），**始终导出全部 83 列**（忽略紧凑列隐藏）；含小计/合计/分区/表头/数据行；**非**全库 `Store.projects`。
- **导出内容：** 保留 Excel **公式**（`auto_calc`、小计 `SUBTOTAL`、合计 `SUM`）、**数字/日期格式**（`ct.fa` → `z`）、**单元格背景/字体/对齐**（`bg/fc/ht` → `s`）、**合并单元格**。
- **依赖：** `index.html` 使用 `xlsx-js-style@1.2.0`（`cellStyles: true`）；导入仍走 `XlsxImporter.importFromFile`。
- **降级：** Luckysheet 未挂载时 `exportBuiltLuckysheetView()` 用 `buildLuckysheetCelldata` + `buildLuckysheetDataMatrix` 重建后导出。
- **文件名：** `buildExportFilename()` → `查看数据_{报告月}_{YYYYMMDD_HHMMSS}.xlsx`；J 版快照「下载此版本」→ `项目执行跟踪_{label}.xlsx`。
- **开发说明：** `docs/设计文档/LUCKYSHEET_XLSX_EXPORT.md`
- **遗留：** `js/xlsx-importer.js#exportToXlsx` 标记 deprecated（纯数据，无样式公式）；报告线服务端 `exportReportLine` 未改。
...
- **入口：** 单击 Luckysheet **F 列（项目号）** 打开右侧 Drawer（1280px，`.project-drawer--split` 限制 `max-width: 100vw`）；F 列禁止内联编辑，显示链接色与下划线。
- **Header：** 品牌色渐变顶栏仅保留项目号、项目名称和项目切换控件 **`←` · `当前序号/总数` · `→`**；客户、PM、执行单位、业务类型和周期下沉至左侧「项目基准」键值行，避免重复展示。
- **双栏主体（`.project-drawer-split`）：**
  1. **左侧项目基准（`.drawer-baseline-panel`）：** 约占 34%，独立滚动；「项目基准」卡由 `baselineIdentityFields` 按 **F/G/H/E/D/K** 顺序以键值行展示（项目号、项目名称、客户、项目经理、执行单位、业务类型），另加 **项目周期** `B ~ C`；**不再**以标签展示 I/J/K，也不展示 A；三张同款 `.drawer-baseline-rate-card` 由 `progressCards` 驱动——合同完成率 `(始累完成)÷P`、开票进度 `(始累开票)÷P`、回款进度 `(始累回款)÷P`；「WIP与应收账款」指标表按 **AL/AJ** 展示（`BASELINE_METRIC_LABELS`）；其下「项目实施进展」卡片绑定 `progressFields`（`LEFT_PROGRESS_COLS=['M']`），可按权限编辑。
  2. **右侧业务 Tab（`.drawer-detail-tabs`）：** 约占 66%，内容区独立滚动；`activeTab` 默认 `forecast`，切换不重建 `draft`。
- **Tab 映射（`ProjectDrawerLayout.DRAWER_TAB_CONFIG`）：**
  - `forecast`：年度完成额申报、完成额统计与预测、开票与回款统计预测；
  - `wip`：WIP 分析与措施；
  - （已移除）`status` / `extended`。
- **经营预测：**
  - `computeDrawerMetrics` 仍计算 KPI（完成合同额/完成开票额/完成回款额：截止上年 `prevYear`、当年累计、预测、`remaining = totalContract − prevYearBase − actual − forecast`，三项 `prevYear`/`prevYearBase` 分别对应 `prev_year_completion` / `prev_year_invoice` / `prev_year_payment`）及左侧完成率等；KPI 金额用 `formatBaselineAmount`（完整原值+千分位）；「剩余」行 `.drawer-forecast-kpi-remain` 高亮；**界面不再展示**顶部摘要条（合同剩余 / 当年预测 / 当期历时）与「年度口径明细」折叠区；
  - 标题为「月度填报」；12 个月矩阵列为完成合同额/完成开票额/完成回款额；`isMatrixCellEditable` 强制开票/回款报告月只读；`matrixCellClass` 对开票/回款报告月使用 `.is-historical`（与历史月同款，无角标）；完成合同额报告月仍为 `.is-current` +「当月」；未来可填格「预测」；
  - 历史月份 `.is-historical`，报告月完成合同额 `.is-current`，未来预测 `.is-forecast`；报告月完成合同额输入继续挂载 `completionAuxVisible` 填报参考 Popover；`.drawer-tab-panel--forecast` 内矩阵区 flex 占满右侧剩余高度。
- **WIP 分析：** 混合只读与可编辑字段；控件继续由 `getFieldWidgetType` 决定 enum/longtext/amount/text。AH 为 0 或空时 WIP Tab 顶部展示无需填写提示，但字段仍可按权限录入。
- **工时与成本 Tab（`timesheet_cost`）：** 独立 Tab；「工时数据」「成本数据」以 `drawer-tab-section` **始终展示**（不再包 `el-collapse`）；组件仍为 `ProjectTimesheetAux` / `ProjectCostAux`。
- **项目预警：** `.project-drawer-alerts--bar` 位于双栏上方，仅 `projectAlerts` 非空时渲染。
...
- **WIP 展示规则：** 当 **`WIP（不含税）`（列 AH / `wip_excl_tax`）为 0 或空** 时，WIP Tab 顶部展示提示；标题旁可显示 **「本项目未产生WIP」**；**默认展开，不自动折叠**。`resetDraft` → `expandAllCollapseSections` 展开全部折叠项。
- **P1 控件：** 含 `enum_values` 的字段 → `el-select`（**I 列企业属性、Y 列税率**已改为自由文本，**无**下拉）；长文本（说明/备注/分析/措施/计划等）→ `textarea`（3 行）。
- **保存：** Drawer 右下角固定「保存」→ 批量 diff → 单次 `PUT` + 公式重算 → `syncLuckysheetProjectRowValues` 就地更新该行单元格值与高亮（不整表 `refreshLuckysheet`）；完成额仅校验有效数字；**S 列约束仅在提交时校验**（§5.6）；切换项目不自动保存未提交编辑。
- **快照/只读：** 历史版本预览或 `canEdit === false` 时 Drawer 仍可查看；右下角「保存」始终展示，通过 `saveDisabled`（`!canEdit`）禁用，悬停提示「当前为只读，无法保存」；`handleSave` 在只读时直接 return。
...
| 标签 | 标识符 | 判定条件 | 数据来源 | 阻断提交 |
|---|---|---|---|---|
| **累计开票超总合同额** | `invoice_exceeds_contract` | AC **>** P（等价 R **< 0**） | 公式重算后的项目数据 | 否 |
| **累计回款超总合同额** | `payment_exceeds_contract` | AF **>** P | 公式重算后的项目数据 | 否 |
| **累计回款超累计开票** | `payment_exceeds_invoice` | AF **>** AC | 公式重算后的项目数据 | 否 |
| **存量合同额为负** | `contract_stock_negative` | S 列（存量合同额 = P − U）**< 0** | 公式重算后的项目数据 | 是（手工完成额编辑实时拦截 S<0；系统同步合同额变更触发自动对冲，见 **§5.6**） |
| **有完成额无工时** | `completion_no_hours` | **报告月**月度完成额 **> 0**，且同月**已审工时** ≈ 0 | 完成额：AV–BG 中 `monthIdx` 对应列；工时：`GET /api/projects/:projectNo/timesheet` 专业×月矩阵**总计行**同月列 | 否（辅助参考） |
| **有工时无完成额** | `hours_no_completion` | **报告月**已审工时 **> 0**，且同月月度完成额 ≈ 0 | 同上 | 否（辅助参考） |

- **阈值：** 金额绝对值 **< 0.005** 视为 0；工时使用 **< 0.05 小时** 视为无工时。
- **工时相关预警加载策略：** Drawer 打开后请求当年工时统计；**请求完成前**不展示「有完成额无工时 / 有工时无完成额」（避免 API 失败或加载中误报）；开票/回款/存量 S 预警**同步计算**，不依赖工时接口。
...
- **存量预警：** 复用 `js/stock-validation.js` 的 `hasInvoiceExceedsContract` / `hasPaymentExceedsContract` / `hasPaymentExceedsInvoice` / `hasContractStockViolation`（R 列单元格红底仍用 `hasInvoiceStockWarning`，与 `invoice_exceeds_contract` 等价）。
...
- **下载此版本（`isViewingJSnapshot`）：** 查看 J 版快照时，快照标签旁显示「**下载此版本**」按钮（`el-icon-download`，size mini）；点击调用 `handleDownloadSnapshot()` → `XlsxImporter.exportToXlsx(snapshotProjects, reportingMonth, filename)`，文件名格式 `项目执行跟踪_{快照label}.xlsx`。`exportToXlsx` 第三参数 `filename` 为可选覆盖。
...
| **月度开票** | 系统调整字段；系统值只读，按授权维护调整值 | 历史只读 | 可编辑，作为未来预测 | `MI_COLS` 中仅 `m > monthIdx` 可写；正式环境当月实际值由开票事件更新 |
| **月度回款** | 系统调整字段；系统值只读，按授权维护调整值 | 历史只读 | 可编辑，作为未来预测 | `MP_COLS` 中仅 `m > monthIdx` 可写；正式环境当月实际值由回款事件更新 |

- **正式目标：** 报告月当月开票 / 回款由业务事件写 `system_value`；获授权用户修正时只写 `adjustment_value`。Demo 仍保留 `_system_ref/_system_override` 兼容结构，待正式集成时迁移。
...
在当前角色、锁定规则下**可填报**的字段（`FieldConfig.canEdit` 且 `canEdit`），其**数据格**使用浅黄底，与「本月有变更」的浅橙底区分；表头仍为浅黄底 + 深黄字（见 **§3.3.3**）。
  ...
  - **范围：** 按字段取值类型与报告月时间窗判断；公式字段、系统直取字段和系统调整字段的 `system_value` 不适用。系统调整字段只有获授权的调整入口可编辑。
...
| 3 | **存在人工调整值** | 浅蓝 `#e8f4fd` + 系统值/调整值/显示值批注（正式目标，§2.2） |
| 4 | 当前可编辑列 | 浅黄 `#fefce8` |
...
flowchart TD
    ...
    Q2 -->|否| Q3{"adjustment_value<br/>非零？"}
    Q3 -->|是| C3["🔵 浅蓝底 #e8f4fd<br/>系统值/调整值批注"]
...
- **完成额编辑校验（已实现）：**
  ...
  2. **合同额事件或人工调整自动对冲：** 合同额事件更新 `system_value`，或获授权用户修改 `adjustment_value` 后导致 **S < 0** 时，系统自动调减报告月完成额并追加「系统自动对冲」变更批注；构表阶段不静默改写 Store 数据。
  ...
  4. **合同额核减场景：** 若业务事件或调整值变化导致 P 降低，从而使全年完成预测超 P，系统**不**自动调减完成额，仅在提交前阻断并提示用户调整未来月份预测或完成 CRB 合同额变更。
  5. **提交阻断兜底：** PM「提交」、板块管理员「提交审批」前校验范围内全部项目 **S ≥ 0**、全年完成预测合计 **≤ P**，且当前月完成额为负时 `completion_remark` 非空；不满足则弹出汇总错误。
  6. **完成额负值与备注（已实现）：**
     - 编辑：`validateCompletionEdit` 禁止未来月完成额 `< 0`；历史月/当前月允许负值。
     - 引导：`shouldGuideNegativeRemark(old, new, field, monthIdx)` 仅在当前月「非负→负」时，由 `ProjectEditor`/`ReportLineDetail`/`ProjectDetailDrawer` 打开抽屉并聚焦 CF 备注。
     - 抽屉保存：前端 `validateNegativeCompletionRemark` 按状态拦截（当前月负且备注空）。
     - 服务端：`server/completion-negative-validation.js` 的 `mode:'save'`（`PUT /api/projects`、报告线 `saveData`）仅禁未来月负值；`mode:'submit'`（主表 PM 提交、报告线 `pmSubmit`/`submitApproval`）额外要求当前月负值备注。
     - 导入不拦备注；改回非负时不清空备注。
  - **提示文案（提交失败）：** 「有 N 个项目存量合同额为负…请调减完成额或完成 CRB 合同变更后再提交。」
  - **提示文案（预测超额）：** 「有 N 个项目已完成额加未来月份预测完成额超过总合同额，请调整完成额预测或完成 CRB 合同额变更后再提交。」
  - **提示文案（负值缺备注）：** 「有 N 个项目当前月份完成合同额为负且未填写备注，请补充备注后再提交。」
- **WIP 催开票校验（已实现）：**
...
- **与业务事件的关系（§2）：** CRB 合同事件或财务开票事件更新后，R/S 随公式重算变化；系统值更新不作为人工填报高亮，但预警色与筛选即时反映。
- **实现：** `js/stock-validation.js`、`js/wip-validation.js`、`server/completion-negative-validation.js`；字段字典列 R/S/AL/AM/AN/AO/CF；`ProjectEditor.js` / `ReportLineDetail.js` / `ProjectDetailDrawer.js`；`test/completion-forecast-validation.test.js`、`test/negative-completion-remark-validation.test.js`。
...
4. **系统管理员「提交归档」** → **仅锁定期可提交**（前端 `canSubmitArchive` 要求 `lockStatus === 'locked'`；后端 `dbm.getEffectiveLockStatus(db) !== 'locked'` 返回 403）；前端先自动保存；append **J 版**（全量）；更新 `baselineVersion`；调用 `reportLineService.completeFinalizingReportLines` 将当前报告月 `finalizing` → `completed`（审批记录 `archive_complete`）；调用 `db.resetWorkflowCycleAfterArchive(db)` **流程态归零**（`pmSubmissions`、`sectorFlows`、`approvalStatus`、`reportingSubmitted`、`companyFlow`、`sectorLatestDVersion`）；**不改** `lockStatus`；可多次提交（无 409 限制）。
...
- **数据加载：** 所有角色进入表单时只加载当前已入库数据，不触发源系统取数；板块管理员视图经 `scopedProjects` 仍仅见本板块。
...
| `js/luckysheet-xlsx-export.js` | Luckysheet `file.data` → xlsx（公式/样式/合并）；`exportSheetToXlsx`、`lsCellToXlsx` |
| `js/xlsx-importer.js` | `importFromFile`；`exportToXlsx` **deprecated**（纯数据导出，填报页不再使用） |
...
- **预警演示项目（已实现）：** 重导/重置后由 `server/alert-demo-seed.js` 自动改写演示项目并注入演示工时，对应 Drawer **§3.7.2** 四类预警；另含同项目多预警样例：

| 预警标签 | 演示项目号 | 说明 |
|---|---|---|
| 存量开票额为负 | `B25001-0042` | 单类型 |
| 存量合同额为负 | `B24264-0038` | 单类型 |
| 有完成额无工时 | `B25126-0003` | 单类型 |
| 有工时无完成额 | `B25340` | 单类型 |
| 同项目多预警 | `B24264-0039`（金山中心 · 宋建生） | 同时命中 R / S / 有完成额无工时；键名 `multi_alert` |

  映射写入 `meta.alertDemoProjectNos`。预警按 `projectNo__alertType` 各存一条；忽略按类型独立。若需同步写回根目录 `初始数据.xlsx`（本地文件，`.gitignore` 忽略），可执行 `npm run patch:init-alerts`。
...
> 全角色可打开的全公司预警聚合面板；忽略/撤回仅系统管理员。对应产品版 §7.4。

#### 7.4.1 功能定位与数据聚合

- **入口：** 查看数据 / 报告线详情 / 审批审阅表工具栏「项目预警」按钮（`el-icon-bell`）；`canShowAlertsButton = !isViewingSnapshot`（全角色）；报告线详情覆盖为 `true`
- **忽略权限：** `canDismissAlerts = user.role === 'system_admin'` → `AlertsDrawer.canDismiss`；`POST .../dismiss|undismiss` 校验 `role === 'system_admin'`
...
2. 调用 `StockValidation` 检查开票/回款/存量预警（`invoice_exceeds_contract`、`payment_exceeds_contract`、`payment_exceeds_invoice`、`contract_stock_negative`）
...
5. 与 DB 同步：新预警 → `active`；已存在活跃 → 更新 `lastSeenAt`；不再出现 → **删除行**（不写 `resolved`）；清理历史 `resolved` 行
6. 返回列表仅含 `active` / `dismissed` 与统计摘要（无 `resolvedCount`）
...
CREATE TABLE IF NOT EXISTS project_alerts (
  ...
  status TEXT NOT NULL DEFAULT 'active',  -- active | dismissed（不再使用 resolved）
  first_detected_at TEXT NOT NULL DEFAULT '',
  resolved_at TEXT NOT NULL DEFAULT '',   -- 保留列兼容旧库，写入恒为空
...
- **条件消失**：`deleteAlert` 直接删除活跃行；`deleteResolvedAlerts` 清理历史 resolved
...
| 文件 | 说明 |
|---|---|
| `js/components/AlertsDrawer.js` | Vue 2 组件，`window.AlertsDrawer` IIFE 模式；卡片底部仅操作区，不展示首次检测时间 |
...
- 四维筛选：关键词搜索 + 预警类型 + 板块 + 状态（`全部` 默认 / `active` / `dismissed`）
...
```json
{
  ...
  "summary": {
    ...
    "activeCount": 4,
    "dismissedCount": 0,
    "byType": { "contract_stock_negative": 1, "invoice_exceeds_contract": 1, "payment_exceeds_invoice": 1, "hours_no_completion": 2 },
    "projectCount": 4
  }
}
```
...
- 条件消失后直接删除活跃行，不保留 `resolved`；历史 `resolved` 在下次聚合时清理
- 重复打开不产生重复记录（UNIQUE + ON CONFLICT）

#### 7.4.7 忽略与撤回（dismiss / undismiss）

系统管理员可手动忽略预警，被忽略的 `(project_no, alert_type)` 组合在任何月份都不再出现在活跃列表；可在「已忽略」中撤回。
...
**响应：**
- dismiss：`{ "ok": true, "dismissal": { "projectNo", "alertType", "dismissedAt", "dismissedBy" } }`
- undismiss：`{ "ok": true, "undismissal": { "projectNo", "alertType", "year", "monthIdx", "removed" } }`；服务端随后重跑 `collectAllAlerts`
...
**撤回逻辑（`undismissAlertById`）：**

1. 校验当前行为 `dismissed`
2. `DELETE` dismissals 对应行
3. 将当前行暂标 `active`（便于条件已消失时被 sync 删除）
4. 调用方重跑 `collectAllAlerts`

**前端：**

- `statusOptions`：`全部`（默认）/ `活跃` / `已忽略`
- 活跃卡片 footer「忽略」；已忽略卡片 footer「已忽略」+「撤回」
- `handleDismiss` / `handleUndismiss`：`$confirm` → POST → `fetchAlerts()`

**实现文件：**

| 文件 | 变更 |
|---|---|
| `server/db.js` | `deleteAlert` / `deleteResolvedAlerts` / dismiss / undismiss |
| `server/alert-service.js` | 条件消失删除；`dismissAlertById` / `undismissAlertById` |
| `server/index.js` | `POST .../dismiss` + `POST .../undismiss` + audit |
| `js/components/AlertsDrawer.js` | 状态筛选三选项；去掉已消除 UI |
| `css/style.css` | `.alerts-card.is-dismissed` |
| `test/alert-undismiss.test.js` | 忽略、撤回、删除清理单测 |
...
**列表顶部截止提醒（`ReportLineList.deadlineReminder`）：**

- 数据源：`Store.reportingMonth`、`Store.periodConfig.deadlineDay`（缺省回退 `lockDay` / `25`）
- 文案示例：`当前报告月 2026年6月，填报截止日为每月 25 日。`
- UI：`.rl-deadline-banner`（单一品牌色提示条，无剩余天数/逾期态）
- 与筛选/分页无关，进入 `/#/report-lines` 即展示

**列表双状态列：**

- UI：`ReportLineList` 列顺序为「我的状态」→「报告线状态」→周期→板块→操作；筛选占位「报告线状态」/「全部报告线状态」
- API：`GET /api/report-lines` 每行附带 `my_status: { code, label, type }`
- 映射：`server/report-line-my-status.js` → `resolveMyStatus(user, line, pmStatusRow)`
  - PM：批量查 `report_line_pm_status`（`open`/`rejected`/无记录→待提交；`submitted`→已提交；`closed`→已关闭）
  - sector_admin / sector_director / group_leader：由 `report_lines.status` 推导（见产品版 §7.5.0a）
  - system_admin 等：`—`
- 前端兜底：`myStatusTag(row)` 优先用 `row.my_status`，缺省时按角色本地映射

#### 7.5.2 文件清单

| 文件 | 变更 |
|---|---|
| `server/report-line-my-status.js` | **新增** `resolveMyStatus` 及各角色映射纯函数 |
...
| `server/report-line-service.js` | `getReportLines` 写入 `my_status`；`reviewApproval`/`autoCompletePeriod` 目标态 `finalizing` 并 `syncReportLineFromMain`；`saveData` 拒绝 `finalizing/completed`；导出 `syncProjectToFinalizingReportLines` / `completeFinalizingReportLines` |
| `server/index.js` | `PUT/POST /api/projects` 保存后同步 finalizing 报告线；`POST /api/company/archive` 调用 `completeFinalizingReportLines`；截止日/强制结束进入核对归档中 |
| `js/store.js` | `queryReportLines`；`getVisibleOperations` 含 `finalizing`；`autoCompleteCurrentReportLines` |
| `js/views/ProjectEditor.js` | `handleSubmitArchive` 先自动保存再确认；有 finalizing 时确认文案提示封账 |
| `js/views/ReportLineList.js` / `ReportLineDetail.js` | 状态文案「核对归档中」；筛选项；流转轨迹标签 |
| `server/report-line-my-status.js` | sector_admin：`finalizing`→核对归档中；总监/群主：`finalizing`→已审批 |
| `test/report-line-finalizing.test.js` | **新增** 覆盖 autoComplete→finalizing、审批、禁写、主表同步、归档封账 |
...
| POST | `/api/report-lines/auto-complete-current` | `system_admin` | 手动触发：当前报告月非 `finalizing/completed` 报告线进入 `finalizing`（核对归档中），并同步主表数据；返回数量、明细与 `state` |
| POST | `/api/company/archive` | `system_admin` | 锁定期生成 J 版；将当前报告月 `finalizing` 封账为 `completed`；响应含 `finalizedReportLines` |
...
#### 7.5.7 报告线详情页统一为查看数据界面

目标：从「填报与审批」进入报告线后，界面应与系统管理员打开「查看数据」保持一致，仅数据范围、可编辑权限、保存/提交/审批 API 不同。
...
| `canEdit` | `fill` 模式且报告线状态为 `open/returned/rejected` 时，PM/板块管理员/系统管理员按权限可编辑；`view/approve/finalizing/completed/closed` 只读 |
...
- **存量合同额约束：** 手工编辑月度完成额时实时拦截会导致 **S < 0** 的输入；正式环境中合同额事件或调整值变化导致 **S < 0** 时自动调减报告月完成额并留「系统自动对冲」批注；提交时兜底校验全部 **S ≥ 0**（§5.6）。Demo 仍通过旧同步结构演示该分支。
...
```mermaid
flowchart TD
    TRIGGER["触发条件<br/>新报告年度开启事件"] --> ACTION["全部原有项目<br/>system_value → 旧项目"]
...
| # | 补充项 | 状态 | 说明 |
|---|---|---|---|
| 1 | **单元格底色区分（数据源可视化）** | Demo 已实现旧覆盖样式；正式目标待重构 | 正式环境中 `adjustment_value !== 0` 使用浅蓝 `#e8f4fd`，批注展示系统值、调整值和显示值（§2.2）。 |
| 2 | **项目详情 Drawer / 枚举·长文本编辑** | ✅ 已实现（P0/P1+） | F 列打开 Drawer（§3.7.1）：Header + 项目切换；浅黄可编辑态；报告月完成额 Popover 参考；金额千分位；分区默认展开（WIP=0 仅提示）；保存回写 Sheet。 |
...
| 8 | **存量指标 R/S 预警与校验** | Demo 已实现；事件接入待重构 | R/S 预警筛选；手工完成额实时拦截 S<0；正式环境由合同额事件/调整值变化触发自动对冲；提交前兜底校验 S≥0。详见 §5.6。 |
...
| **新、旧项目（A 列）** | 区分历史遗留 vs 新增 | 正式目标为 `system_direct`（§2.3）：当年准入事件新增默认为「新项目」，跨报告年初 rollover 为「旧项目」，任何人不可编辑。与 §5.3.1「新增项目行」高亮不是同一概念 |
| **R 列 / S 列** | 总合同额-开票 / 总合同额-完成 | 已更名为 **存量开票额（R）** / **存量合同额（S）**；分区名 **存量指标**（§5.6） |
| **当年调整值（O）/ 总合同额（P）** | O=P−N；P 直取 CRB | **O** 正式目标为系统调整字段：CRB3+CRB5 合同变更额系统值 + 人工调整值；**P=N+O显示值**（§2.4） |
