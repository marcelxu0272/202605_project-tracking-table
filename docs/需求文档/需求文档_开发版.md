...
- **板块管理员配置：** 系统自动获取平台项目执行板块（当前由 `sectorRegistry` 提供）；系统管理员为每个板块从平台用户下拉中维护 `adminName` / `adminUserId`。
- **总监审批跳过：** 不在管理页手工配置；板块管理员提交审批时，后端根据 `sectorAdmins[sectorCode].adminUserId/adminName` 匹配平台用户，并判断该用户是否具备同板块 `sector_director` 权限。若是，则直接把该板块 `approvalStatus` 置为 `approve1`，即跳过板块总监初审。
...
| 层级 | 路径 / 接口 | 说明 |
|---|---|---|
| 前端 UI | `js/views/AdminSettings.js` | 系统用户卡片按 `sectorRegistry` 渲染两列表格：板块、板块管理员；板块管理员为平台用户下拉筛选 |
...
| 保存接口 | `PATCH /api/admin/users` | 接收 `sectorAdmins` 并仅保存 `adminName` / `adminUserId`；旧 `users`/`groupRegistry` 字段保留兼容 |
