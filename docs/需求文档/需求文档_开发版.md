...
> **最后更新：** 2026-05-29（调整存量合同额校验与自动对冲边界 §5.6）
...
- **完成额编辑校验（已实现）：**
  ...
  3. **提交阻断兜底：** PM「提交」、板块管理员「提交审批」前校验范围内全部项目 **S ≥ 0**；不满足则弹出汇总错误。
...
- 填报页底部显示「**本月已提交 PM**」面板（本板块 PM，`status === 'submitted'` 或历史 `received`），含「**查看变更**」；标题栏提供**收起 / 展开**（默认展开，对齐系统管理员「各板块进度」底栏）。
