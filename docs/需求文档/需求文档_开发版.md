...
- **批量刷新（已实现框架，开发期 stub）：**
  - **手动刷新（已实现）：** **系统管理员**与**板块管理员**在填报页点击「**刷新数据**」→ `POST /api/editor/refresh-data`：① 工程平台引用同步（新增项目、工程平台引用列 `_system_ref`、报告月 MI/MP；不覆盖 N/T 初始化导入基准）；② 重载 SQLite 中**最新项目 payload**（含各 PM 已提交/已保存数据、板块管理员改数、审批驳回后恢复的数据）。**不**覆盖未被管理员覆盖的系统引用显示值（§2.2）。
