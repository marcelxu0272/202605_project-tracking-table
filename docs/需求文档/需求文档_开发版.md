...
> **最后更新：** 2026-07-14（左侧完成率卡补全总合同额/始累名称）
...
- **双栏主体（`.project-drawer-split`）：**
  1. **左侧项目基准（`.drawer-baseline-panel`）：** 约占 34%，独立滚动；元信息卡展示 H/E/D 与 A/I/J/K 标签，**项目周期**单行展示 `B ~ C`（`headerDateRange`）；完成率卡按 **(始累完成)÷P×100%** 计算，左侧大字百分比，右侧标注「项目始累完成合同额」「总合同额」金额；指标表按 **S/R/AC/AF/AP/AL** 顺序展示（P/U 仅在完成率卡，不重复），R/S 负值继续复用 `.drawer-stock-warn`。
...
- **经营预测：**
  - `computeDrawerMetrics` 仍计算 KPI（完成合同额/开票/回款：当年累计、预测、`remaining = totalContract − actual − forecast`）及左侧完成率等；KPI 金额用 `formatBaselineAmount`（完整原值+千分位）；「剩余」行 `.drawer-forecast-kpi-remain` 高亮；**界面不再展示**顶部摘要条（合同剩余 / 当年预测 / 当期历时）与「年度口径明细」折叠区；
