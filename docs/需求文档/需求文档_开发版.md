...
> **最后更新：** 2026-06-08（填报管理重构及详情页统一：AppLayout/AdminSettings 移除全局锁定控件 CSS；db.js 扩充 12 板块配置及 GRP_ZB；seed 补 2026-05；ReportLineList 按 LIST_FORM 重设计；ReportLineDetail.js 继承 ProjectEditorView 完整模板并复用 Luckysheet 体验；PM 范围过滤、历史月份查看/导出、板块管理员全部 PM 提交情况）
...
系统管理员通过「填报管理」页右上角的「手动发起填报」按钮，弹窗预览当前报告月的各板块审批链路人员，确认后为所有尚未存在的 `(sector_code, period)` 批量创建 `open` 状态报告线，数据来自最新 J 版快照。

#### 7.5.2 文件清单

| 文件 | 变更 |
|---|---|
| `server/report-line-service.js` | 新增 `resolveApprovalStaffForSector`、`getForkPreview`；增强 `forkPeriod`（J 版取数、审计日志、无 J 版 400） |
| `server/index.js` | 新增 `GET /api/report-lines/fork-preview`；增强 `POST /api/report-lines/fork-period` 响应；报告线导出接口接收 `role/pmName/sectorCode` 查询参数 |
| `js/store.js` | 新增 `fetchForkPreview()`、`forkReportPeriod(period)`；`getVisibleOperations()` 对历史月份统一返回 `view/export` |
| `js/views/ReportLineList.js` | 页头增加「手动发起填报」按钮（`system_admin` 可见）+ 预览弹窗；导出时透传当前用户身份参数 |
| `js/views/ProjectEditor.js` | 模板增加报告线扩展钩子：`rlContextBar`、`canShowEditorSaveGroup`、`rlCanSubmit`、`rlCanApprove`、`rlCanReject`、`rlSubmitting` 与 `handleRl*` 默认空方法；报告线信息与操作按钮同处业务工具栏一行 |
| `js/views/ReportLineDetail.js` | `extends: window.ProjectEditorView`，不自定义模板；完整复用项目追踪表 Luckysheet 界面，仅覆盖报告线数据源、权限、保存、提交、审批、导出；`scopedProjects` 按当前角色过滤，PM 仅看到本人项目 |
...
`forkConfirmDisabled = !baselineAvailable || will_create === 0`
