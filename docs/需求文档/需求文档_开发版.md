...
> **最后更新：** 2026-07-28（合同与存量指标去掉始累开票/回款）
...
- **文件名：** `buildExportFilename()` → `项目追踪表_{报告月}_{YYYYMMDD_HHMMSS}.xlsx`；J 版快照「下载此版本」→ `项目执行追踪_{label}.xlsx`。
...
- **Header：** 品牌色渐变顶栏仅保留项目号、项目名称和项目切换控件 **`←` · `当前序号/总数` · `→`**；客户、标签、PM、执行单位和周期下沉至左侧项目基准面板，避免重复展示。
- **双栏主体（`.project-drawer-split`）：**
  1. **左侧项目基准（`.drawer-baseline-panel`）：** 约占 34%，独立滚动；元信息卡展示 H/E/D 与 A/I/J/K 标签，**项目周期**单行展示 `B ~ C`（`headerDateRange`）；三张同款 `.drawer-baseline-rate-card` 由 `progressCards` 驱动——合同完成率 `(始累完成)÷P`、开票进度 `(始累开票)÷P`、回款进度 `(始累回款)÷P`，左侧大字百分比，右侧分别标注「始累完成合同额/始累开票/始累回款」与「总合同额」；指标表按 **S/R/AL/AJ** 顺序展示（`BASELINE_METRIC_LABELS` 将 AL/AJ 覆盖为 WIP、应收账款；AC/AF 仅在进度卡展示并写入 `RATE_CARD_METRIC_COLS` 去重），R/S 负值继续复用 `.drawer-stock-warn`。
...
- **Tab 映射（`ProjectDrawerLayout.DRAWER_TAB_CONFIG`）：**
  ...
  - `status`：合同签署与进展；
  - `wip`：WIP 分析与措施；
  - `extended`：合同额、存量指标、始累完成合同额、开票回款情况、财务数据、应收账款及 WIP（左侧已使用的关键列去重）。
- **经营预测：**
  - `computeDrawerMetrics` 仍计算 KPI（完成合同额/开票/回款：当年累计、预测、`remaining = totalContract − prevYearBase − actual − forecast`，三项 `prevYearBase` 分别对应 `prev_year_completion` / `prev_year_invoice` / `prev_year_payment`）及左侧完成率等；KPI 金额用 `formatBaselineAmount`（完整原值+千分位）；「剩余」行 `.drawer-forecast-kpi-remain` 高亮；**界面不再展示**顶部摘要条（合同剩余 / 当年预测 / 当期历时）与「年度口径明细」折叠区；
  - 标题为「月度填报」；12 个月矩阵列为完成合同额/开票/回款；`isMatrixCellEditable` 强制开票/回款报告月只读；`matrixCellClass` 对开票/回款报告月使用 `.is-historical`（与历史月同款，无角标）；完成合同额报告月仍为 `.is-current` +「当月」；未来可填格「预测」；
...
- **项目状态 / WIP 分析：** 混合只读与可编辑字段；控件继续由 `getFieldWidgetType` 决定 enum/longtext/amount/text。AH 为 0 或空时 WIP Tab 顶部展示无需填写提示，但字段仍可按权限录入。
- **延伸数据：** 其余只读 section 使用三列 `el-descriptions`；**不再包含**工时/成本。
...
| `server/report-line-service.js` | `getReportLines` 写入 `my_status`（PM 批量查 pm_status）；另含 `normalizeDistributedColumns`、`syncMonthlyFlatFields`、`forkPeriod`/`getReportLineDetail` 等既有逻辑 |
...
| `js/views/ReportLineList.js` | 列表「我的状态」+「报告线状态」列与 `myStatusTag`；筛选文案改为报告线状态；弹窗分发列配置；截止提醒条；流转轨迹等 |
| `test/report-line-my-status.test.js` | **新增** 覆盖 PM 三态、管理员、总监/群主四态、系统管理员 — |
| `js/views/AdminSettings.js` | 填报周期配置恢复 `deadlineDay`（截止填报日期），新增「自动结束当前所有报告线」测试按钮 |
| `js/store.js` | `DEFAULT_CONFIG` 含 `deadlineDay: 25`；`forkReportPeriod(period, distributedColumns)` 更新签名，支持透传列配置；新增 `fetchForkPreview()`、`autoCompleteCurrentReportLines()`；`getVisibleOperations()` 对历史月份统一返回 `view/export` |
...
| `css/style.css` | 新增 `.fork-dialog *` 轻量样式；新增 `.rl-deadline-banner` 列表顶部截止提醒条 |
