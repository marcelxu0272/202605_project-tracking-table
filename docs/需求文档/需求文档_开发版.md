...
| **manual_input（WIP 分析 AM–AU）** | 同上 | 不受月份窗限制；WIP 为 0 或空时仍默认展开，并提示「本项目未产生WIP」 |
| **项目号 F** | 全部 R | 运行期不可编辑 |
...
- **双栏主体（`.project-drawer-split`）：**
  1. **左侧项目基准（`.drawer-baseline-panel`）：** 约占 34%，独立滚动；元信息卡展示 H/E/D 与 A/I/J/K 标签，**项目周期**单行展示 `B ~ C`（`headerDateRange`）；完成率卡按 **(始累完成)÷P×100%** 计算，左侧大字百分比，右侧标注「项目始累完成合同额」「总合同额」金额；指标表按 **S/R/AC/AF/AL/AJ** 顺序展示（P/U 仅在完成率卡，不重复；`BASELINE_METRIC_LABELS` 将 AC/AF/AL/AJ 抽屉展示名覆盖为始累开票、始累回款、WIP、应收账款），R/S 负值继续复用 `.drawer-stock-warn`。
...
| `js/views/ReportLineList.js` | 弹窗新增「分发列配置」区域：`forkDistMode`（all/custom）、`forkSelectedCols`；computed `forkColumnGroups`/`forkSelectedCount`；methods `isMandatoryForkCol`/`toggleForkCol`/`selectAllForkCols`/`clearForkCols`；`confirmFork` 传 `distributedCols`；导出时透传当前用户身份参数；流转轨迹支持 `auto_complete` 节点；**列表顶部 `deadlineReminder` 截止日提醒条**（读 `Store.reportingMonth` + `periodConfig.deadlineDay`） |
