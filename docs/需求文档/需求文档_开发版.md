...
> **最后更新：** 2026-06-08（截止填报日期与报告线自动完结：`deadlineDay` 配置、`autoCompletePeriod`、手动测试接口与管理设置按钮）
...
| `server/report-line-service.js` | 新增 `normalizeDistributedColumns`（强制 E/F/G，去重排序）；`forkPeriod` 接收 `options.distributedColumns`，写入 DB；`getReportLineDetail`/`getReportLines` 返回 `distributed_columns`（JSON 解析）；`exportReportLine` 按分发列过滤导出字段（内嵌 `COL_TO_KEY`）；新增 `resolveApprovalStaffForSector`、`getForkPreview`、`autoCompletePeriod` |
