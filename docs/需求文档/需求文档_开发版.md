...
> **最后更新：** 2026-06-08（填报管理重构及详情页统一：AppLayout/AdminSettings 移除全局锁定控件 CSS；db.js 扩充 12 板块配置及 GRP_ZB；seed 补 2026-05；ReportLineList 按 LIST_FORM 重设计；ReportLineDetail.js 继承 ProjectEditorView 完整模板并复用 Luckysheet 体验）
...
| `server/index.js` | 新增 `GET /api/report-lines/fork-preview`；增强 `POST /api/report-lines/fork-period` 响应 |
| `js/store.js` | 新增 `fetchForkPreview()`、`forkReportPeriod(period)` |
| `js/views/ReportLineList.js` | 页头增加「手动发起填报」按钮（`system_admin` 可见）+ 预览弹窗 |
...
| `js/views/ReportLineDetail.js` | `extends: window.ProjectEditorView`，不自定义模板；完整复用项目追踪表 Luckysheet 界面，仅覆盖报告线数据源、权限、保存、提交、审批、导出 |
...
| `scopedProjects` | 返回当前报告线 `tableProjects`；报告线列表入口已按角色过滤，详情页不对报告线项目再做主表二次过滤 |
